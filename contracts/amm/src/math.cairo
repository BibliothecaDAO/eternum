...
pub fn get_input_price(
    ...
    let input_amount_after_fee = (input_amount * (fee_denom - fee_num)) / fee_denom;
...
pub fn get_output_price(
    ...
    let denominator = (output_reserve - output_amount) * (fee_denom - fee_num);
...
pub fn compute_lp_mint(lords_added: u256, lords_reserve: u256, total_lp_supply: u256) -> u256 {
    if total_lp_supply == 0 {
        ...
        (lords_added * total_lp_supply) / lords_reserve
    }
}
...
pub fn compute_lp_burn(
    ...
    let lords_out = (lp_amount * lords_reserve) / total_lp_supply;
    let token_out = (lp_amount * token_reserve) / total_lp_supply;
...
#[cfg(test)]
mod tests {
    use super::{compute_add_liquidity, compute_lp_burn, compute_lp_mint, get_input_price, get_output_price, quote};
    ...
    fn test_compute_lp_mint_first() {
        let lp = compute_lp_mint(1000, 0, 0);
        assert!(lp == 1000, "first mint should equal lords amount");
    }
