...
export function processData(fileName) {
  ...
  const setTraitTypesNameCalldata = generateTraitTypesCalldata(
    traitsTypeIdToNamesMap,
    dataOverwriteMap,
    onlyProcessOverwriten,
  );
  const setTraitValueNameCalldata = generateTraitValuesCalldata(
    traitsValuesToNamesMap,
    dataOverwriteMap,
    onlyProcessOverwriten,
  );
  const setAttrsRawToIPFSCIDCalldata = generateAttrsRawCalldata(
    traitsCombinedIpfsCid,
    traitsTypeNameToIdMap,
    traitsValuesNameToIdMap,
    onlyProcessOverwriten,
  );
  const setDefaultIpfsCidCalldata = generateDefaultIpfsCidCalldata(
    defaultIpfsCid,
    defaultIpfsCidOverwrite,
    onlyProcessOverwriten,
  );
  const setMintCalldata = generateMintCalldata(
    mintCData,
    traitsTypeNameToIdMap,
    traitsValuesNameToIdMap,
    onlyProcessOverwriten,
...
function validateTraitTypeStructure(trait, expectedTraitTypeId) {
  ...
  if (expectedTraitTypeId != trait.traitTypeId) {
    throw new Error(
      `Incorrectly ordered traits: Trait type id mismatch: ${expectedTraitTypeId} != ${trait.traitTypeId}, ${trait}`,
    );
  }
...
function validateTraitValueStructure(traitValue, expectedTraitValueId) {
  if (expectedTraitValueId != traitValue.traitValueId) {
    throw new Error(
      `Incorrectly ordered traits: Trait value id mismatch: ${expectedTraitValueId} != ${traitValue.traitValueId}, ${traitValue}`,
    );
  }
}
...
function generateTraitValuesCalldata(traitsValuesToNamesMap, dataOverwriteMap, onlyProcessOverwriten) {
  ...
  for (const [key, traitValueName] of traitsValuesToNamesMap.entries()) {
    ...
    if (onlyProcessOverwriten) {
      ...
      if (overwrite) {
        setTraitValueNameCalldata.push([
          traitTypeId,
          traitValueId,
          byteArray.byteArrayFromString(traitValueName),
          overwrite,
        ]);
      }
    } else {
 ...
 */
function generateAttrsRawCalldata(
  traitsCombinedIpfsCid,
  traitsTypeNameToIdMap,
  traitsValuesNameToIdMap,
  onlyProcessOverwriten,
...
function getAttrsRawFromTraitsString(traitsString, traitsTypeNameToIdMap, traitsValuesNameToIdMap) {
  ...
  traits.forEach((trait) => {
    ...
    if (traitTypeId === undefined || traitValueId === undefined) {
      throw new Error(
        `Invalid trait data: traitTypeId=${traitTypeId} (${typeof traitTypeId}), traitValueId=${traitValueId} (${typeof traitValueId}) for trait ${traitTypeName}:${traitValueName}`,
      );
    }
...
function logProcessedData(data, fileName, onlyProcessOverwriten) {
  ...
  if (setTraitTypesNameCalldata.length > 0) {
    ...
    setTraitTypesNameCalldata.forEach(([typeId, byteArrayName, overwrite]) => {
      ...
      console.log(
        `   │ ${typeId.toString().padStart(3)} │ ${nameStr.padEnd(55)} │ ${overwrite ? "✓" : "✗"} ${overwrite ? "       " : "       "} │`,
      );
    });
    console.log("   └─────┴─────────────────────────────────────────────────────────┴───────────┘");
  }
  ...
  if (setTraitValueNameCalldata.length > 0) {
    ...
    setTraitValueNameCalldata.forEach(([typeId, valueId, byteArrayName, overwrite]) => {
      const nameStr = byteArray.stringFromByteArray(byteArrayName);
      console.log(
        `   │ ${typeId.toString().padStart(3)} │ ${valueId.toString().padStart(3)} │ ${nameStr.padEnd(51)} │ ${overwrite ? "✓" : "✗"} ${overwrite ? "       " : "       "} │`,
      );
    });
    console.log("   └─────┴─────┴─────────────────────────────────────────────────────┴───────────┘");
  }
  ...
  if (setAttrsRawToIPFSCIDCalldata.length > 0) {
    ...
    setAttrsRawToIPFSCIDCalldata.forEach(([attrsRaw, byteArrayCid, overwrite]) => {
      ...
      console.log(
        `   │ ${attrsStr} │ ${cidStr.padEnd(47)} │ ${overwrite ? "✓" : "✗"} ${overwrite ? "       " : "       "} │`,
      );
    });
    console.log("   └──────────────────┴─────────────────────────────────────────────────┴───────────┘");
  }
  ...
  if (setMintCalldata.length > 0) {
    ...
    setMintCalldata.forEach(([toAddress, _, attrsRawAndCount]) => {
      console.log(
        `   │ ${attrsRawAndCount["0"]}  │ ${toAddress.padEnd(16)}        │ ${attrsRawAndCount["1"].toString().padEnd(47)} │`,
      );
    });
    console.log("   └──────────────────┴─────────────────────────────────────────────────┴───────────┘");
  }
  ...
  console.log("┌─────────────────────────────────────────────────────────────────────────────┐");
  console.log(
    `│ Total Trait Types Updated:          ${setTraitTypesNameCalldata.length.toString().padStart(8)} entries              │`,
  );
  console.log(
    `│ Total Trait Values Updated:         ${setTraitValueNameCalldata.length.toString().padStart(8)} entries              │`,
  );
  console.log(
    `│ Total Attribute Mappings Updated:   ${setAttrsRawToIPFSCIDCalldata.length.toString().padStart(8)} combinations        │`,
  );
  console.log(
    `│ Total Mint Calldata Generated:      ${setMintCalldata.length.toString().padStart(8)} entries              │`,
  );
  console.log(
    `│ Processing Mode:            ${(onlyProcessOverwriten ? "Overwrite Only" : "Full Processing").padEnd(24)} │`,
  );
  console.log("└─────────────────────────────────────────────────────────────────────────────┘");
