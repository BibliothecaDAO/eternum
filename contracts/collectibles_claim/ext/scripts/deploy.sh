...
#
# DESCRIPTION:
#   This script handles the deployment process for the collectible claim smart 
#   contracts. It manages the build process using Scarb and executes the 
#   deployment using Bun.
#
# USAGE:
#   ./deploy.sh [network]  - Builds and deploys the collectible claim contracts
...
echo -e "${GREEN}► Installing Collectible Claim deployment dependencies...${NC}"
cd deployment
bun install
...
# Execute deployment
echo -e "${GREEN}► Executing collectible claim deployment script...${NC}"
bun run deploy:${NETWORK}

echo -e "\n${GREEN}✔ collectible claim deployment process completed on ${NETWORK}${NC}\n"