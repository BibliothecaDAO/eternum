...
mod tests {
    ...
    use collectibles_claim::tests::mocks::contracts::{
        IMockPaymentERC721Dispatcher, IMockPaymentERC721DispatcherTrait, MockCollectibleERC721, MockPaymentERC721,
        MockVrfProvider,
    };
    ...
    use snforge_std::{
        ContractClass, ContractClassTrait, DeclareResultTrait, EventSpyAssertionsTrait, declare, spy_events,
        start_cheat_block_timestamp, start_cheat_caller_address, start_cheat_chain_id_global,
        start_cheat_transaction_hash, stop_cheat_block_timestamp, stop_cheat_caller_address,
    ...
    fn setup_claim_scenario() -> (ContractAddress, ContractAddress, ContractAddress, ContractAddress) {
        ...
        let (vrf_provider, _) = declare("MockVrfProvider").unwrap().contract_class().deploy(@array![]).unwrap();
...
//     let collectible_token = IERC721Dispatcher { contract_address: collectible_contract };
//     assert!(collectible_token.balance_of(alice) == 3, "Alice should have received collectibles with zero VRF
//     provider");
// }
