...
impl BuildingProductionImpl of BuildingProductionTrait {
    ...
    fn update_bonuses_received(self: @Building, world: IWorldDispatcher, sign: bool) {
        ...
        let building_coord: Coord 
            = Coord {x: *self.inner_col, y: *self.inner_row};
        let mut building_production: Production 
            = get!(world, (self.outer_entity_id, resource_type), Production);
    ...
    fn _update_bonus_received_from(self: @Building, ref production: Production, inner_coord: Coord, world: IWorldDispatcher, sign:bool) {
        let building_at_coord: Building 
            = get!(world, (self.outer_col, self.outer_row, inner_coord.x, inner_coord.y), Building);
        if sign {
            production
                .increase_boost_percentage(
                    building_at_coord.category.get_production_multiplier());
        } else {
            production
                .decrease_boost_percentage(
                    building_at_coord.category.get_production_multiplier());
        }
    }
    ...
    fn _update_bonus_supplied_to(
        ...
        if produced_resource_type.is_non_zero(){
            ...
            if sign {
                production
                    .increase_boost_percentage(
                        self.category.get_production_multiplier());
            } else {
            production
                .decrease_boost_percentage(
                    self.category.get_production_multiplier());
