...
fn test_create_bank() {
        ...
        .create_bank(
            ...
            array![
                (
                    ...
                    ResourceTypes::LORDS,
                    array![(ResourceTypes::WHEAT, 500), (ResourceTypes::FISH, 200)].span()
                ),
                (// lords costs 700 DRAGONHIDE
                ResourceTypes::LORDS, array![(ResourceTypes::DRAGONHIDE, 700),].span()),
            ]
                .span()
        );
    ...
    let bank_shekel_wheat_cost = get!(
        world, (bank_shekel_swap_cost_1.resource_cost_id, 0), ResourceCost
    );
    assert(bank_shekel_wheat_cost.amount == 500, 'wrong wheat cost');

    let bank_shekel_fish_cost = get!(
        world, (bank_shekel_swap_cost_1.resource_cost_id, 1), ResourceCost
    );
    ...
    assert(bank_shekel_swap_cost_2.resource_cost_count == 1, 'wrong resource cost count');

    let bank_shekel_dragonhide_cost = get!(
...
fn test_set_bank_auction() {
    ...
    let bank_id: u128 = 1;
    set!(world, Bank { entity_id: bank_id, exists: true });
    ...
    bank_config_dispatcher
        .set_bank_auction(
            bank_id,
            array![(ResourceTypes::LORDS, 0), (ResourceTypes::DRAGONHIDE, 0)].span(),
            decay_constant,
            per_time_unit,
