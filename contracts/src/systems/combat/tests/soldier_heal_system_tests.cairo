...
fn setup() -> (IWorldDispatcher, u128, u128, ISoldierSystemsDispatcher) {
    ...
    combat_config_dispatcher
        .set_soldier_config(
            world, array![// pay for each soldier with the following
    ...
    combat_config_dispatcher
        .set_health_config(
            world, SOLDIER_ENTITY_TYPE, array![// pay for a unit of health with the following
            (ResourceTypes::DEMONHIDE, 17)].span(), 100
        );
    ...
    set!(
        world,
        (Resource { entity_id: caller_id, resource_type: ResourceTypes::DEMONHIDE, balance: 5000 })
    );
...
fn test_heal_soldier() {
    ...
    set!(world, (Health { entity_id: new_unit_id, value: 60 }));
    ...
    soldier_systems_dispatcher.heal_soldiers(world, new_unit_id, health_bought);
    ...
    let caller_demonhide_balance = get!(world, (caller_id, ResourceTypes::DEMONHIDE), Resource);
    assert!(
