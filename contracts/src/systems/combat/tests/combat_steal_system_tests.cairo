...
fn setup() -> (IWorldDispatcher, u128, u128, u128, u128, ICombatSystemsDispatcher) {
    ...
    combat_config_dispatcher.set_attack_config(SOLDIER_ENTITY_TYPE, 100);
    combat_config_dispatcher.set_defence_config(SOLDIER_ENTITY_TYPE, 100);
    combat_config_dispatcher.set_health_config(SOLDIER_ENTITY_TYPE, array![].span(), 100);
    ...
    IWeightConfigDispatcher { contract_address: config_systems_address }
        .set_weight_config(TYPE_ONE_RESOURCE_TO_BE_STOLEN_FROM_TARGET.into(), 200);
    ...
    let attacker_unit_id = soldier_systems_dispatcher
        .create_soldiers(attacker_realm_entity_id, ATTACKER_SOLDIER_COUNT);
    ...
    let target_unit_id = soldier_systems_dispatcher
        .create_soldiers(target_realm_entity_id, TARGET_SOLDIER_COUNT);

    // add target unit to town watch
    soldier_systems_dispatcher
        .merge_soldiers(
            target_town_watch_id, array![(target_unit_id, TARGET_SOLDIER_COUNT),].span()
        );
...
fn test_steal_success() {
    ...
    // steal from target
    combat_systems_dispatcher.steal(attacker_unit_id, target_town_watch_id);
...
fn test_not_owner() {
    let (_, _, attacker_unit_id, _, target_town_watch_id, combat_systems_dispatcher) = setup();
...
fn test_steal_success_army_to_army() {
    ...
    // steal from target
    combat_systems_dispatcher.steal(attacker_unit_id, target_unit_id);
