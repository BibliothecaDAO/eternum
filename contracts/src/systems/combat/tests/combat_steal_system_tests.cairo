...
fn setup() -> (IWorldDispatcher, u128, u128, u128, u128, ICombatSystemsDispatcher) {
    ...
    combat_config_dispatcher.set_attack_config(world, SOLDIER_ENTITY_TYPE, 100);
    combat_config_dispatcher.set_defence_config(world, SOLDIER_ENTITY_TYPE, 100);
    combat_config_dispatcher.set_health_config(world, SOLDIER_ENTITY_TYPE, array![].span(), 100);
    ...
    IWeightConfigDispatcher { contract_address: config_systems_address }
        .set_weight_config(world, TYPE_ONE_RESOURCE_TO_BE_STOLEN_FROM_TARGET.into(), 200);

    IWeightConfigDispatcher { contract_address: config_systems_address }
        .set_weight_config(world, TYPE_TWO_RESOURCE_TO_BE_STOLEN_FROM_TARGET.into(), 200);
    ...
    let attacker_realm_entity_id = realm_systems_dispatcher
        .create(
            world,
            realm_id,
    ...
    starknet::testing::set_contract_address(contract_address_const::<'target'>());

    let target_realm_entity_id = realm_systems_dispatcher
        .create(
            world,
            realm_id,
            resource_types_packed,
            ...
            wonder,
            0,
            target_realm_entity_position.clone(),
        );
    ...
    set!(
        ...
        (
            Owner {
                entity_id: attacker_realm_entity_id, address: contract_address_const::<'attacker'>()
            },
            Resource {
    ...
    set!(
        world,
        (Owner { entity_id: target_realm_entity_id, address: contract_address_const::<'target'>() })
    );
    ...
    let attacker_unit_id = soldier_systems_dispatcher
        .create_soldiers(world, attacker_realm_entity_id, ATTACKER_SOLDIER_COUNT);
    ...
    let target_unit_id = soldier_systems_dispatcher
        .create_soldiers(world, target_realm_entity_id, TARGET_SOLDIER_COUNT);

    // add target unit to town watch
    soldier_systems_dispatcher
        .merge_soldiers(
            world, target_town_watch_id, array![(target_unit_id, TARGET_SOLDIER_COUNT),].span()
        );
...
fn test_steal_success() {
    ...
    set!(world, (Defence { entity_id: target_town_watch_id, value: 0 }));

    starknet::testing::set_contract_address(contract_address_const::<'attacker'>());

    // steal from target
    combat_systems_dispatcher.steal(world, attacker_unit_id, target_town_watch_id);
    ...
    let target_realm_wheat_resource = get!(
        world, (target_realm_entity_id, ResourceTypes::WHEAT), Resource
    );
    assert!(
        target_realm_wheat_resource.balance == INITIAL_RESOURCE_BALANCE
            - (WHEAT_BURN_PER_SOLDIER_DURING_ATTACK * ATTACKER_SOLDIER_COUNT),
        "wrong wheat value"
    );

    let target_realm_fish_resource = get!(
        world, (target_realm_entity_id, ResourceTypes::FISH), Resource
    );
    assert!(
        target_realm_fish_resource.balance == INITIAL_RESOURCE_BALANCE
            - (FISH_BURN_PER_SOLDIER_DURING_ATTACK * ATTACKER_SOLDIER_COUNT),
        "wrong fish value"
    );
    ...
    let attacker_resource_chest_foreign_key = InternalInventorySystemsImpl::get_foreign_key(
        attacker_inventory, 0
    );
    let attacker_resource_chest_id = get!(world, attacker_resource_chest_foreign_key, ForeignKey)
        .entity_id;

    // check that resource chest in inventory is filled
    let attacker_resource_chest_weight = get!(world, attacker_resource_chest_id, Weight);
    ...
    let stolen_resource_types: Array<u8> = array![
        TYPE_ONE_RESOURCE_TO_BE_STOLEN_FROM_TARGET, TYPE_TWO_RESOURCE_TO_BE_STOLEN_FROM_TARGET,
    ];
    let mut index = 0;
    loop {
...
fn test_steal_success_with_order_boost() {
    ...
    set!(world, (Orders { order_id: 1, hyperstructure_count: 1 }));
...
fn test_not_owner() {
    let (world, _, attacker_unit_id, _, target_town_watch_id, combat_systems_dispatcher) = setup();
...
fn test_attacker_in_transit() {
    ...
    set!(world, (ArrivalTime { entity_id: attacker_unit_id, arrives_at: 1001 }));
...
fn test_attacker_dead() {
    ...
    set!(world, (Health { entity_id: attacker_unit_id, value: 0 }));
...
fn test_wrong_position() {
    ...
    set!(world, (Position { entity_id: attacker_unit_id, x: 100, y: 200 }));
...
fn test_steal_success_army_to_army() {
    ...
    let target_unit_position = Position {
        entity_id: target_unit_id, x: attacker_unit_position.x, y: attacker_unit_position.y,
    };
    set!(world, (target_unit_position));

    let target_unit_loot_id = 1007;
    let mut target_unit_inventory = Inventory {
        entity_id: target_unit_id, items_key: 'some value', items_count: 0
    };

    let mut attacker_unit_inventory = Inventory {
        entity_id: attacker_unit_id, items_key: 'anor value', items_count: 0
    };

    set!(world, (attacker_unit_inventory, target_unit_inventory));

    // set attacker and defender capacity
    let attacker_capacity = Capacity { entity_id: attacker_unit_id, weight_gram: 500 };

    let defender_capacity = Capacity { entity_id: target_unit_id, weight_gram: 500 };
    set!(world, (attacker_capacity, defender_capacity));

    target_unit_inventory.add_test_item(world, target_unit_loot_id);

    set!(world, (Defence { entity_id: target_unit_id, value: 0 }));

    starknet::testing::set_contract_address(contract_address_const::<'attacker'>());

    // steal from target
    combat_systems_dispatcher.steal(world, attacker_unit_id, target_unit_id);
