...
use eternum::models::resources::{
    ...
    ResourceTypes, RESOURCE_PRECISION
};
use eternum::constants::{WORLD_CONFIG_ID, ARMY_ENTITY_TYPE, TickIds};
use eternum::utils::testing::{
    world::spawn_eternum,
    systems::deploy_realm_systems,
    systems::deploy_config_systems,
    systems::deploy_combat_systems
};
use eternum::systems::{
...
fn mint(world: IWorldDispatcher, entity: u128, mut resources: Span<(u8, u128)>) {
    loop {
        match resources.pop_back(){
            Option::Some((_type, amount)) => {
                let mut resource 
                    = ResourceCustomImpl::get(world, (entity, *_type));
                resource.add(*amount);
                resource.save(world);
            },
            Option::None => {break;}
...
fn setup() -> (
    ...
    starknet::testing::set_contract_address(
        contract_address_const::<REALMS_OWNER>()
    );

    let realm_id = realm_system_dispatcher.create(
            1,1,1,1,1,1,1,1,1, 
            Position {
                entity_id: 0,
                x:REALM_COORD_X,
    ...
    (
        world, combat_system_dispatcher,
        realm_id,
    )
}
...
fn test_army_create_not_owner() {
    ...
    starknet::testing::set_contract_address(
        contract_address_const::<'someone_else'>()
    );
    combat_systems_dispatcher
        .army_create(realm_id, false);
}
