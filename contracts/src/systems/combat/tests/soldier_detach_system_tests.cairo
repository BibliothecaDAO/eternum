...
fn setup() -> (IWorldDispatcher, u128, u128, ISoldierSystemsDispatcher) {
    ...
    let caller_realm_entity_id = realm_systems_dispatcher
        .create(
            realm_id,
    ...
    set!(
        world,
        (Resource { entity_id: caller_id, resource_type: ResourceTypes::WHEAT, balance: 5000 })
    );
    set!(
        world,
        (Resource { entity_id: caller_id, resource_type: ResourceTypes::WOOD, balance: 5000 })
    );
    ...
    let new_unit_id = soldier_systems_dispatcher.create_soldiers(caller_id, num_soldiers_bought);
...
fn test_detach_unit() {
    let (world, caller_id, unit_id, soldier_systems_dispatcher) = setup();

    starknet::testing::set_contract_address(contract_address_const::<'caller'>());

    // detach 7 soldiers from unit
    let num_detached_soldiers = 7;
    let new_unit_id = soldier_systems_dispatcher.detach_soldiers(unit_id, num_detached_soldiers);
    ...
    assert(
        new_unit_position.x == caller_position.x && new_unit_position.y == caller_position.y,
        'wrong position'
    );
...
fn test_not_owner() {
    let (_, _, unit_id, soldier_systems_dispatcher) = setup();
    ...
    // detach 7 soldiers from unit
    let num_detached_soldiers = 7;
    soldier_systems_dispatcher.detach_soldiers(unit_id, num_detached_soldiers);
}
...
fn test_single_soldier_detach() {
    let (world, _, unit_id, soldier_systems_dispatcher) = setup();

    set!(world, (Quantity { entity_id: unit_id, value: 1 }));
...
fn test_unit_has_items_in_inventory() {
    ...
    set!(world, (Inventory { entity_id: unit_id, items_key: 1, items_count: 1 }));
