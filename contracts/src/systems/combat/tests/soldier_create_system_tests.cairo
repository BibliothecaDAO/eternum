...
use eternum::models::combat::{Attack, Health, Defence, TownWatch};
...
fn setup() -> (IWorldDispatcher, u128, ISoldierSystemsDispatcher) {
    ...
    set!(
        world, (Owner { entity_id: realm_entity_id, address: contract_address_const::<'caller'>() })
    );

    set!(
        world,
        (Resource {
            entity_id: realm_entity_id, resource_type: ResourceTypes::WHEAT, balance: 5000
        })
    );
    set!(
        world,
        (Resource { entity_id: realm_entity_id, resource_type: ResourceTypes::WOOD, balance: 5000 })
    );
...
fn test_create_soldier() {
    ...
    let new_unit_id = soldier_systems_dispatcher
        .create_soldiers(realm_entity_id, num_soldiers_bought);
...
fn test_not_owner() {
    let (_, realm_entity_id, soldier_systems_dispatcher) = setup();
    ...
    soldier_systems_dispatcher.create_soldiers(realm_entity_id, num_soldiers_bought);
}
...
fn test_not_realm() {
    let (_, _, soldier_systems_dispatcher) = setup();
