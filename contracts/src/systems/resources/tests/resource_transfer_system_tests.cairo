mod resource_transfer_system_tests {
    ...
    fn test_transfer() {
        ...
        let receiver_inventory = get!(world, receiver_entity_id, Inventory);
        assert!(receiver_inventory.items_count == 1, "wrong inventory item count");

        let received_item_foreign_key = InternalInventorySystemsImpl::get_foreign_key(
            receiver_inventory, 0
        );

        let received_item_foreign_key = get!(world, received_item_foreign_key, ForeignKey);
    ...
    fn test_transfer__not_owner() {
        ...
        resource_systems_dispatcher
            .transfer(
                world, 1, 2, array![(ResourceTypes::STONE, 400), (ResourceTypes::WOOD, 700),].span()
            );
    }
    ...
    fn test_transfer__entity_position_mismatch() {
        ...
        let sender_entity_id = 11_u64;
        let sender_entity_position = Position {
            x: 900_000, y: 800_000, entity_id: sender_entity_id.into()
        };
        ...
        let receiver_entity_id = 12_u64;
        let receiver_entity_position = Position {
            x: 500_000, y: 600_000, entity_id: receiver_entity_id.into()
        };
        set!(world, (receiver_entity_position));
