mod inventory_transfer_system_tests {
    ...
    fn setup() -> (IWorldDispatcher, u128, u128, u128, IInventorySystemsDispatcher) {
        ...
        set!(
            ...
            (
                Inventory {
                    entity_id: donor_transport_id, items_key: world.uuid().into(), items_count: 0
                },
                Capacity { entity_id: donor_transport_id, weight_gram: 80000000 }
            )
        );
    ...
    fn test_inventory_transfer_item() {
        let (world, chest_id, _, donor_transport_id, inventory_systems_dispatcher) = setup();

        starknet::testing::set_contract_address(contract_address_const::<'donor'>());

        let receiver_id = 700;
        inventory_systems_dispatcher.transfer_item(donor_transport_id, 0, receiver_id);

        // check receiver balance
        let receiver_stone_resource = get!(world, (receiver_id, ResourceTypes::STONE), Resource);
        let receiver_gold_resource = get!(world, (receiver_id, ResourceTypes::GOLD), Resource);
        ...
        let donor_transport_inventory = get!(world, donor_transport_id, Inventory);
        assert(donor_transport_inventory.items_count == 0, 'wrong donor transport inventory');

        // check donor transport weight
        let donor_transport_weight = get!(world, donor_transport_id, Weight);
    ...
    fn test_inventory_transfer_item_not_transport_owner() {
        let (_, _, _, donor_transport_id, inventory_systems_dispatcher) = setup();
    ...
    fn test_inventory_transfer_item_wrong_position() {
        let (world, _, _, donor_transport_id, inventory_systems_dispatcher) = setup();
        ...
        set!(
            world,
            (
                Position { entity_id: donor_transport_id, x: 1, y: 1 },
                Position { entity_id: receiver_id, x: 2, y: 2 }
            )
        );
    ...
    fn test_inventory_transfer_item_wrong_arrival_time() {
        ...
        set!(
            world,
            (ArrivalTime {
                entity_id: donor_transport_id, arrives_at: starknet::get_block_timestamp() + 1000
            })
        );
