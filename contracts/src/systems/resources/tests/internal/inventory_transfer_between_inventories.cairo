mod internal_transfer_between_inventories_tests {
    ...
    fn player_inventories(world: IWorldDispatcher) -> (Inventory, Inventory) {
        ...
        let player_A_capacity = Capacity {
            entity_id: player_A_inventory.entity_id,
            weight_gram: PLAYER_CAPACITY
        };

        let player_B_capacity = Capacity {
            entity_id: player_B_inventory.entity_id,
            weight_gram: PLAYER_CAPACITY
        };
    ...
    fn test_transfer_between_inventories_scenerio_1() {
        ...
        let (mut player_A_inventory, mut player_B_inventory) 
            = player_inventories(world);

        // add 4 items to player A inventory
        player_A_inventory
            .add_test_item(world, 'A0'.try_into().unwrap());
        player_A_inventory
            .add_test_item(world, 'A1'.try_into().unwrap());
        player_A_inventory
            .add_test_item(world, 'A2'.try_into().unwrap());
        player_A_inventory
            .add_test_item(world, 'A3'.try_into().unwrap());
        
        // add 1 item to player B inventory
        player_B_inventory
            .add_test_item(world, 'B0'.try_into().unwrap());
    ...
    fn test_transfer_between_inventories_indexes_only_up_to_capacity() {
        ...
        player_A_inventory
            .add_test_item(world, 'A3'.try_into().unwrap());
        player_A_inventory
            .add_test_item(world, 'A4'.try_into().unwrap());
        player_A_inventory
            .add_test_item(world, 'A5'.try_into().unwrap());
        player_A_inventory
            .add_test_item(world, 'A6'.try_into().unwrap());
        player_A_inventory
            .add_test_item(world, 'A7'.try_into().unwrap());
    

        InternalInventorySystemsImpl::transfer_between_inventories(
            world, player_A_inventory, player_B_inventory, 
            array![0, 1, 2, 3, 4, 5, 6, 7].span()
        );
