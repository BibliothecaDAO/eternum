...
mod donkey_systems {
    ...
    impl InternalDonkeySystemsImpl of InternalDonkeySystemsTrait {
        fn create_donkey(
            ...
            let mut donkeys: Resource 
                = ResourceImpl::get(world, (payer_id, ResourceTypes::DONKEY));
            ...
            let arrives_at: u64 = starknet::get_block_timestamp() 
                + InternalDonkeySystemsImpl::get_donkey_travel_time(
                    world, start_coord, intermediate_coord, donkey_speed_config.sec_per_km, is_round_trip);
            let delivery_coord: Coord = if is_round_trip {start_coord} else {intermediate_coord};
