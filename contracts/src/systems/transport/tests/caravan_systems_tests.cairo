...
fn setup() -> (IWorldDispatcher, Array<u128>, ICaravanSystemsDispatcher, u128) {
    ...
    IWorldConfigDispatcher { contract_address: config_systems_address }
        .set_world_config(
            contract_address_const::<0>(), world.contract_address
        ); // realm l2 contract address
    ...
    let first_free_transport_unit_id = transport_unit_systems_dispatcher
        .create_free_unit(realm_entity_id, 10);
    let second_free_transport_unit_id = transport_unit_systems_dispatcher
        .create_free_unit(realm_entity_id, 10);
    let transport_units: Array<u128> = array![
        first_free_transport_unit_id, second_free_transport_unit_id
    ];
...
fn test_create_caravan() {
    let (world, transport_units, caravan_systems_dispatcher, realm_entity_id) = setup();

    // create caravan
    let caravan_id = caravan_systems_dispatcher.create(transport_units.clone());
    ...
    let (
        ...
        caravan_position,
        caravan_owner,
        caravan_entity_owner
    ) =
        get!(
        world, caravan_id, (CaravanMembers, Movable, Capacity, Position, Owner, EntityOwner)
    );
...
fn test_create_caravan__not_owner() {
    let (_, transport_units, caravan_systems_dispatcher, _) = setup();
...
fn test_create_caravan__blocked_entity() {
    let (_, mut transport_units, caravan_systems_dispatcher, _) = setup();
...
fn test_disassemble_caravan() {
    let (world, transport_units, caravan_systems_dispatcher, _) = setup();
    ...
    // disassemble caravan
    let disassembled_transport_ids = caravan_systems_dispatcher.disassemble(caravan_id);
    ...
    let foreign_key_1: Array<felt252> = array![
        caravan_id.into(), original_caravan_members.key.into(), 0
    ];
    let foreign_key_1: felt252 = poseidon_hash_span(foreign_key_1.span());
    let first_transport = get!(world, foreign_key_1, ForeignKey);
    assert(first_transport.entity_id.into() == 0, 'foreign key is set');

    let foreign_key_2: Array<felt252> = array![
        caravan_id.into(), original_caravan_members.key.into(), 1
    ];
    let foreign_key_2: felt252 = poseidon_hash_span(foreign_key_2.span());
    let second_transport = get!(world, foreign_key_2, ForeignKey);
...
fn test_disassemble_caravan__caller_not_owner() {
    ...
    // disassemble caravan
    starknet::testing::set_contract_address(contract_address_const::<0x99>());
    caravan_systems_dispatcher.disassemble(caravan_id);
}
...
fn test_disassemble_caravan__not_caravan() {
    let (_, _, caravan_systems_dispatcher, _) = setup();
