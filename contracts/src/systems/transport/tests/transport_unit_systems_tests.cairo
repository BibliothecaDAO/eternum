...
fn test_create_free_transport_unit() {
    ...
    assert_eq!(quantity.value, 10_u128, "free transport unit not created");

    assert_eq!(position.x, 20, "position not set");
    assert_eq!(position.y, 30, "position not set");

    assert_eq!(metadata.entity_type, FREE_TRANSPORT_ENTITY_TYPE.into(), "entity type not set");

    assert_eq!(owner.address, starknet::get_caller_address(), "owner not set");
    ...
    assert_eq!(quantity_tracker.count, 10, "quantity tracker not updated");
    ...
    let quantity_tracker = get!(world, quantity_tracker_key, QuantityTracker);
    assert_eq!(quantity_tracker.count, 20, "quantity tracker not updated"); 
}
...
#[test]
#[should_panic(expected:  "not enough free transport unit")]
...
fn test_return_free_transport_unit() {
    ...
    let quantity_tracker_key = poseidon_hash_span(quantity_tracker_arr.span());
    let quantity_tracker = get!(world, quantity_tracker_key, QuantityTracker);
    assert_eq!(quantity_tracker.count, 20, "quantity tracker not updated 1");
    ...
    assert_eq!(quantity.value, 0, "unit not returnd");

    assert_eq!(position.x, 0, "position is set");
    assert_eq!(position.y, 0, "position is set");

    assert_eq!(metadata.entity_type, 0, "entity type is set");

    assert_eq!(owner.address, Zeroable::zero(), "owner is set");

    assert_eq!(capacity.weight_gram, 0, "capacity is set");

    assert_eq!(movable.sec_per_km, 0, "speed is set");

    // check that the number of transport units in the realm == 10 after deletion
    let quantity_tracker = get!(world, quantity_tracker_key, QuantityTracker);
    assert_eq!(quantity_tracker.count, 10, "quantity tracker not updated 2");
}


#[test]
#[should_panic(expected:  "not a free transport unit")]
...
#[test]
#[should_panic(expected:  "unit not owned by caller")]
...
#[test]
#[should_panic(expected:  "unit is blocked")]
...
#[test]
#[should_panic(expected:  "unit has no quantity")]
