...
fn test_create_free_transport_unit() {
    ...
    let free_transport_unit_id = transport_unit_systems_dispatcher
        .create_free_unit(realm_entity_id, 10);

    // check that the free transport unit has been created
    let (quantity, position, metadata, owner, capacity, movable, arrival_time) = get!(
        world,
        free_transport_unit_id,
        (Quantity, Position, EntityMetadata, Owner, Capacity, Movable, ArrivalTime)
    );
...
fn test_create_unit__not_owner() {
    let (_, realm_entity_id, transport_unit_systems_dispatcher) = setup();
...
fn test_create_unit__not_enough_free_transport_unit() {
    ...
    // create free transport unit
    transport_unit_systems_dispatcher.create_free_unit(realm_entity_id, 70);
}
...
fn test_return_free_transport_unit() {
    ...
    let free_transport_unit_id_1 = transport_unit_systems_dispatcher
        .create_free_unit(realm_entity_id, 10);
    transport_unit_systems_dispatcher.create_free_unit(realm_entity_id, 10);
    ...
    transport_unit_systems_dispatcher.return_free_units(array![free_transport_unit_id_1].span());

    // check that the free transport unit 1 has been returned
    let (quantity, position, metadata, owner, capacity, movable, _) = get!(
        world,
        free_transport_unit_id_1,
        (Quantity, Position, EntityMetadata, Owner, Capacity, Movable, ArrivalTime)
    );
...
fn test_return__wrong_unit_type() {
    ...
    transport_unit_systems_dispatcher.return_free_units(array![free_transport_unit_id].span());
}
