...
use eternum::systems::config::interface::{
    IBankConfigDispatcher, IBankConfigDispatcherTrait, IWeightConfigDispatcher,
    IWeightConfigDispatcherTrait,
};

use eternum::systems::bank::contracts::bank_systems;
use eternum::systems::bank::interface::{IBankSystemsDispatcher, IBankSystemsDispatcherTrait};
...
fn setup() -> (IWorldDispatcher, u128, u128, IBankSystemsDispatcher,) {
    ...
    IWeightConfigDispatcher { contract_address: config_systems_address }
        .set_weight_config(ResourceTypes::LORDS.into(), 2);
    ...
    let bank_id = bank_config_dispatcher
        .create_bank(
            bank_coord,
            array![
                (
                ...
                ( // lords cost 2 dragonhide
                    ResourceTypes::LORDS, array![(ResourceTypes::DRAGONHIDE, 2),].span()
                ),
            ]
                .span()
        );
    ...
    set!(
        world,
        (
            Owner { entity_id: transport_id, address: contract_address_const::<'transport'>() },
            Inventory { entity_id: transport_id, items_key: world.uuid().into(), items_count: 0 },
            Position { entity_id: transport_id, x: bank_coord.x, y: bank_coord.y },
            Resource { entity_id: transport_id, resource_type: ResourceTypes::WHEAT, balance: 500 },
            Resource { entity_id: transport_id, resource_type: ResourceTypes::FISH, balance: 500 },
            Resource { entity_id: transport_id, resource_type: ResourceTypes::COAL, balance: 500 },
            Resource {
                entity_id: transport_id, resource_type: ResourceTypes::DRAGONHIDE, balance: 500
            },
        )
    );
...
fn test_bank_swap_for_wheat_fish_and_coal() {
    ...
    starknet::testing::set_contract_address(contract_address_const::<'transport'>());

    let lords_bought = 80;

    let initial_transport_weight = get!(world, transport_id, Weight);

    bank_systems_dispatcher.swap(bank_id, 0, transport_id, ResourceTypes::LORDS, lords_bought);
...
fn test_bank_swap_for_dragonhide() {
    ...
    bank_systems_dispatcher.swap(bank_id, 1, transport_id, ResourceTypes::LORDS, lords_bought);
...
fn test_bank_swap__wrong_caller() {
    let (_, bank_id, transport_id, bank_systems_dispatcher) = setup();

    starknet::testing::set_contract_address(contract_address_const::<'unknown_caller'>());
...
fn test_bank_swap__wrong_transport_position() {
    ...
    set!(world, (Position { entity_id: transport_id, x: 0, y: 0 }));
...
fn test_bank_swap__wrong_transport_arrival_time() {
    ...
    set!(world, (ArrivalTime { entity_id: transport_id, arrives_at: 1 }));
...
fn test_bank_swap__no_auction() {
    ...
    bank_systems_dispatcher
        .swap(bank_id, 0, transport_id, ResourceTypes::DRAGONHIDE, dragonhide_bought);
}
