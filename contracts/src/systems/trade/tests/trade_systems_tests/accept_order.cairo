...
fn setup(
    ...
    let taker_first_free_transport_unit_id = transport_unit_systems_dispatcher
        .create_free_unit(world, taker_id, 10);
    ...
    let taker_transport_id = caravan_systems_dispatcher.create(world, taker_transport_units);
...
fn test_accept_without_taker_transport_id() {
    ...
    // accept order 
    starknet::testing::set_contract_address(contract_address_const::<'taker'>());
    trade_systems_dispatcher.accept_order(world, taker_id, 0, trade_id);
...
fn test_accept_without_taker_transport_id_wrong_position() {
    ...
    let (world, trade_id, _, taker_id, _, trade_systems_dispatcher) = setup(false);
