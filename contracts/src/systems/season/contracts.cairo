...
mod season_systems {
    ...
    use eternum::{
        ...
        models::{config::{HyperstructureConfig, ResourceBridgeFeeSplitConfig}, season::{Leaderboard, LeaderboardEntryCustomImpl, LeaderboardEntry}},
        systems::{
    ...
    impl SeasonSystemsImpl of super::ISeasonSystems<ContractState> {
        ...
        fn claim_leaderboard_rewards(ref self: ContractState, token: ContractAddress) {
			...
			if leaderboard.total_price_pool.is_none() {
				...
				let total_price_pool: u256 = ERC20ABIDispatcher { contract_address: token }.balance_of(resource_bridge_fee_split_config.season_pool_fee_recipient);
				leaderboard.total_price_pool = Option::Some(total_price_pool);
			}

			assert!(leaderboard.total_price_pool.is_some(), "If that happens, no one has registered to the leaderboard");
			...
			let percentage_scaled: u256 = (entry.points.into() * SCALING_FACTOR / leaderboard.total_points.into()).into();

			let player_reward: u256 = leaderboard.total_price_pool.unwrap() * percentage_scaled / SCALING_FACTOR;
			let resource_bridge_fee_split_config: ResourceBridgeFeeSplitConfig = world.read_model(WORLD_CONFIG_ID);

			ERC20ABIDispatcher { contract_address: token }.transfer_from(resource_bridge_fee_split_config.season_pool_fee_recipient, caller_address, player_reward);
        }
    }
}
