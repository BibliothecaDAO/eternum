...
use eternum::utils::testing::{
    world::spawn_eternum, systems::{deploy_system, deploy_realm_systems, deploy_hyperstructure_systems, deploy_season_systems},
...
fn setup() -> (WorldStorage, ID, IHyperstructureSystemsDispatcher, ISeasonSystemsDispatcher, starknet::ContractAddress) {
	...
	let resource_bridge_systems_dispatcher = SetupImpl::deploy_resource_bridge_systems(ref world);
...
fn season_test_register_to_leaderboard_success() {
    ...
    let (hyperstructures_contributed_to, hyperstructure_shareholder_epochs) = finish_season(ref world, realm_entity_id, hyperstructure_systems_dispatcher);


	season_systems_dispatcher.register_to_leaderboard(hyperstructures_contributed_to, hyperstructure_shareholder_epochs);
	...
	let leaderboard_entry: LeaderboardEntry = LeaderboardEntryCustomImpl::get(ref world, contract_address_const::<'player1'>());
	assert!(leaderboard_entry.points > 0, "Leaderboard entry points should be greater than 0");

	assert!(leaderboard_entry.points == leaderboard.total_points, "Leaderboard entry points should be equal to leaderboard total points");
}

#[test]
#[should_panic(expected: ("Claiming period hasn't started yet", 'ENTRYPOINT_FAILED'))]
fn season_test_claim_too_early() {
    let (mut world, realm_entity_id, hyperstructure_systems_dispatcher, season_systems_dispatcher, mock_erc20_address) = setup();
...
fn season_test_claim_success() {
	...
	let balance: u256 = ERC20ABIDispatcher { contract_address: mock_erc20_address }.balance_of(contract_address_const::<'player1'>());
...
fn finish_season(ref world: WorldStorage, realm_entity_id: ID, hyperstructure_systems_dispatcher: IHyperstructureSystemsDispatcher) -> (Span<ID>, Span<(ID, u16)>) {
    starknet::testing::set_contract_address(contract_address_const::<'player1'>());
    starknet::testing::set_account_contract_address(contract_address_const::<'player1'>());
    ...
    hyperstructure_systems_dispatcher
        .end_game(hyperstructures_contributed_to, hyperstructure_shareholder_epochs);
