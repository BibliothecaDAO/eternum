...
do
    commands+=(
        ...
        "sozo model get Resource $ADMIN_BANK_ACCOUNT_ENTITY_ID,253"
    )
done

# donkeys
commands+=(
    "sozo execute $LIQUIDITY_SYSTEMS add --account-address $DOJO_ACCOUNT_ADDRESS --calldata $ADMIN_BANK_ENTITY_ID,249,$((DONKEYS_LIQUIDITY * $RESOURCE_PRECISION)),$((LORDS_LIQUIDITY_PER_RESOURCE * $RESOURCE_PRECISION))"
)
