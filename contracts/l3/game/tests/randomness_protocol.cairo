...
fn recorded_roots_preserve_current_game_derivation() {
    let input = read_txt(@FileTrait::new("../randomness-protocol/tests/fixtures/v1.txt"));
    ...
    for _ in 0..count {
        ...
        let root = u256 { low: (*envelope.at(9)).try_into().unwrap(), high: (*envelope.at(10)).try_into().unwrap() };
        for (salt, bound, expected) in draws {
            assert!(random(root, salt, bound) == expected, "game derivation differs");
        }
    }
