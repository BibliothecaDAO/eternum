...
use s1_eternum::models::resource::r3esource::{
    StructureSingleR33esourceFoodImpl, SingleR33esource, SingleR33esourceImpl
};
...
impl ProductionImpl of ProductionTrait {
    ...
    fn harvest(ref resource: SingleR33esource, ref entity_weight: W3eight, resource_weight: u128, current_tick: u32) {
        // get start tick before updating last updated tick
        let mut production: Production = resource.production;
        let start_tick = production.last_updated_tick;

        // last updated tick must always be updated
        production.set_last_updated_tick(current_tick.try_into().unwrap());

        // ensure lords can not be produced
        if resource.resource_type == ResourceTypes::LORDS {return;}
        // stop if production is not active
        if !production.has_building() {return;}

        // total amount of time resources were produced for
        let num_ticks_produced: u128 = current_tick.into() - start_tick.into();
        let mut total_produced_amount = num_ticks_produced * production.production_rate.into();
        ...
        resource.add(total_produced_amount, ref entity_weight, resource_weight);
...
//         let mut from_resource = SingleR33esourceImpl::get(ref world, (from_entity_id, from_resource_type));
//         from_resource.spend(from_resource_amount);
...
//         let mut labor_resource = SingleR33esourceImpl::get(ref world, (from_entity_id, ResourceTypes::LABOR));
...
//         let mut labor_resource = SingleR33esourceImpl::get(ref world, (from_entity_id, ResourceTypes::LABOR));
//         labor_resource.spend(labor_amount);
//         labor_resource.save(ref world);
...
//         SingleR33esourceFoodImpl::pay(ref world, from_entity_id, wheat_burn_amount, fish_burn_amount);
...
//         let mut produced_resource = SingleR33esourceImpl::get(ref world, (from_entity_id, produced_resource_type));
...
//                 let other_resource_cost: SingleR33esourceCost = world.read_model((other_resources_id, i));
...
//                 let mut other_resource = SingleR33esourceImpl::get(ref world, (from_entity_id, other_resource_type));
//                 other_resource.spend(other_resource_amount * production_tick_count);
