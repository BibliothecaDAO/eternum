...
pub mod series_chest_reward_calculator {
    ...
    const TOTAL_NUM_CHESTS_TO_BE_DISTRIBUTED: u128 = 584;
    const CAP_RATIO_BPS: u128 = 11_000; // 1.10
    const EXPECTED_PLAYERS_START: u128 = 24;
    const EXPECTED_PLAYERS_END: u128 = 24;
    ...
    pub impl SeriesChestRewardStateImpl of SeriesChestRewardStateTrait {
        ...
        fn total_spent(self: @SeriesChestRewardState) -> u128 {
            ...
            let over_spent = ((TOTAL_NUM_CHESTS_TO_BE_DISTRIBUTED * (CAP_RATIO_BPS - BPS)) / BPS)
                - *self.overspend_remaining;
            soft_spent + over_spent
        }

        fn supplies(self: @SeriesChestRewardState) -> (u128, u128) {
        ...
        fn allocate_chests(ref self: SeriesChestRewardState, observed_players: u128) -> u128 {
            ...
            if n_obs < MIN_EFFECTIVE_PLAYERS {
                n_obs = MIN_EFFECTIVE_PLAYERS;
            }
            ...
            let curr_est_players = div_round(
                w_sched * scheduled_current + div_round((BPS - w_sched) * self.ema_players_scaled, BPS), BPS,
            );
            let curr_est_players_nonzero = if curr_est_players >= 1_u128 {
                curr_est_players
            } else {
                1_u128
            };
            ...
            let mut growth_ratio_bps = if denom_gr == 0_u128 {
                BPS
            } else {
                div_round(recent_sum * BPS, denom_gr)
            };
            if curr_est_players_nonzero < 1_u128 {
                growth_ratio_bps = BPS;
            }
            ...
            if scale_bps < SCALE_MIN_BPS {
                scale_bps = SCALE_MIN_BPS;
            }
            if scale_bps > SCALE_MAX_BPS {
                scale_bps = SCALE_MAX_BPS;
            }

            // Baseline per-player rate in bps
            let r_budget_bps = if expected_remaining_players == 0_u128 {
                0_u128
            } else {
                (self.soft_supply * scale_bps) / expected_remaining_players
            };
            ...
            if self.has_last_rate {
                ...
                if r_capped_bps > up_limit {
                    r_capped_bps = up_limit;
                }
                if r_capped_bps < down_limit {
                    r_capped_bps = down_limit;
                }
            }
            ...
            if pot <= self.soft_supply {
                ...
                self
                    .overspend_remaining =
                        if self.overspend_remaining > from_over {
                            self.overspend_remaining - from_over
                        } else {
                            0_u128
                        };
            }
            ...
            let tpp_bps = if n_obs == 0_u128 {
                0_u128
            } else {
                div_round(pot * BPS, n_obs)
            };
