...
pub trait IArtificerSystems<TContractState> {
    fn burn_research_for_relic(ref self: TContractState, game_id: u32, structure_id: ID);
}
...
pub mod artificer_systems {
    ...
    impl ArtificerSystemsImpl of super::IArtificerSystems<ContractState> {
        fn burn_research_for_relic(ref self: ContractState, game_id: u32, structure_id: ID) {
            ...
            let artificer_config: ArtificerConfig = WorldConfigUtilImpl::get_member(
                world, game_id, selector!("artificer_config"),
            );
            ...
            let research_weight_grams = ResourceWeightImpl::grams(ref world, game_id, ResourceTypes::RESEARCH);
            ...
            let vrf_seed: u256 = rng_library_dispatcher
                .get_random_number(game_id, Source::Nonce(structure_owner), world);
            ...
            let mut structure_weight = WeightStoreImpl::retrieve(ref world, game_id, structure_id);
            let relic_weight_grams = ResourceWeightImpl::grams(ref world, game_id, relic_type);
            let mut relic = SingleResourceStoreImpl::retrieve(
                ref world, game_id, structure_id, relic_type, ref structure_weight, relic_weight_grams, true,
            );
            ...
            world
                .emit_event(
                    @BurnResearchForRelicEvent {
                        game_id, structure_id, relic_type, timestamp: starknet::get_block_timestamp(),
                    },
                );
        }
    }
