...
pub mod prize_distribution_systems {
    ...
    use crate::models::rank::{PlayerRank, PlayersRankFinal, PlayersRankTrial, RankList, RankPrize, RankPrizeImpl};
    ...
    pub impl PrizeDistributionSystemsImpl of IPrizeDistributionSystems<ContractState> {
        fn blitz_get_or_compute_series_chest_reward_state(ref self: ContractState) -> SeriesChestRewardState {
            ...
            let mut current_game_series_chest_reward_state: SeriesChestRewardState = world.read_model(WORLD_CONFIG_ID);
            ...
            let (series_name, series_game_number) = game_factory_dispatcher
                .get_series_game_data(starknet::get_contract_address());
            ...
            if series_game_number > 1 {
                ...
                last_game_last_prize_distribution_systems = game_factory_dispatcher
                    .get_series_game_address_by_class_hash(series_name, series_game_number - 1, this_class_hash);
            }

            let mut last_game_series_chest_reward_state: SeriesChestRewardState =
                if last_game_last_prize_distribution_systems
                .is_non_zero() {
                IPrizeDistributionSystemsSafeDispatcher { contract_address: last_game_last_prize_distribution_systems }
                    .blitz_get_or_compute_series_chest_reward_state()
                    .unwrap_or(SeriesChestRewardStateImpl::new())
            } else {
            ...
            if current_game_series_chest_reward_state.game_index > last_game_series_chest_reward_state.game_index {
                ...
                let num_chests_allocated = current_game_series_chest_reward_state.allocate_chests(n_obs.into());
        ...
        fn blitz_prize_claim_no_game(ref self: ContractState, registered_player: ContractAddress) {
            ...
            let player_rank_final = PlayersRankFinal { world_id: WORLD_CONFIG_ID.into(), trial_id: SYSTEM_TRIAL_ID };

            let player_rank = PlayerRank { trial_id: SYSTEM_TRIAL_ID, player: registered_player, rank: 1, paid: true };
            ...
            let rank_list = RankList { trial_id: SYSTEM_TRIAL_ID, rank: 1, index: 0, player: registered_player };
        ...
        fn blitz_prize_player_rank(
            ...
            let mut blitz_fee_split_record: BlitzFeeSplitRecord = WorldRecordImpl::get_member(
                world, selector!("blitz_fee_split_record"),
            );
        ...
        fn blitz_get_winner(ref self: ContractState) -> Option<u256> {
            ...
            let players_rank_final: PlayersRankFinal = world.read_model(WORLD_CONFIG_ID);
            if players_rank_final.trial_id.is_zero() {
                return Option::None;
            }
