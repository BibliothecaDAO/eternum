...
/// Hyperstructures are large-scale collaborative buildings that players can contribute resources to,
/// earn points from, and share ownership of through a shareholder system.
///
/// ## Key Formulas
///
...
trait IHyperstructureSystems<T> {
    ...
    ///
    /// This function must be called by the hyperstructure owner before any contributions can be made.
    /// It spends a specified amount of Earthen Shards from the hyperstructure's balance to activate it.
    ///
    /// # Arguments
    ...
    ///
    /// Players can contribute multiple resource types in a single transaction. Resources are taken from
    /// the specified structure and added to the hyperstructure's construction progress. The player earns
    /// construction points proportional to their contribution.
    ///
    /// # Arguments
    ...
    ///
    /// Shares determine what percentage of points generated by the hyperstructure each player receives.
    /// The hyperstructure owner can redistribute shares at any time, which takes effect immediately.
    ///
    /// # Arguments
    /// * `hyperstructure_id` - The ID of the hyperstructure
    /// * `shareholders` - A list of (address, percentage) pairs, where percentage is in basis points (0-10000)
    ///
    /// # Requirements
