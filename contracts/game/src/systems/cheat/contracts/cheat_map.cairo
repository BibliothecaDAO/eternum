...
pub trait ICheatMapSystems<T> {
    fn change_owner(ref self: T, structure_id: ID, to: starknet::ContractAddress);
...
#[starknet::interface]
pub trait ICheatMapSystems2<T> {
    fn explore_buildings(ref self: T, from_structure_id: ID, for_village: bool, for_village_direction: Direction);
}

#[starknet::interface]
pub trait ICheatMapSystems3<T> {
    fn explore_mine_circles(ref self: T, from_structure_id: ID, distance: u8);
}
...
pub mod cheat_map_systems {
    ...
    use s1_eternum::models::config::{CombatConfigImpl, TroopLimitConfig, TroopStaminaConfig, WorldConfigUtilImpl};
    ...
    impl CheatMapSystemsImpl of super::ICheatMapSystems<ContractState> {
        fn change_owner(ref self: ContractState, structure_id: ID, to: starknet::ContractAddress) {
            let mut world: WorldStorage = self.world(DEFAULT_NS());
            // assert_caller_is_admin(world);

            let structure: StructureBase = StructureBaseStoreImpl::retrieve(ref world, structure_id);
            structure.assert_exists();
            StructureOwnerStoreImpl::store(to, ref world, structure_id);
        }
        ...
        fn explore_village_resource_arrivals(ref self: ContractState, structure_id: ID, direction: Direction) {
            ...
            for travel_time in travel_times {
                ...
                ResourceArrivalImpl::slot_increase_balances(
                    ref arrival_resources_array, resources.span(), ref resource_arrival_total_amount,
                );
...
pub mod cheat_map_systems_2 {
    ...
    use s1_eternum::models::weight::{WeightImpl};
    // use s1_eternum::systems::config::contracts::config_systems::{assert_caller_is_admin};
    use s1_eternum::systems::utils::map::IMapImpl;
    use s1_eternum::systems::utils::{mine::iMineDiscoveryImpl, troop::{iExplorerImpl, iGuardImpl, iTroopImpl}};
    ...
    #[abi(embed_v0)]
    impl CheatMapSystems2Impl of super::ICheatMapSystems2<ContractState> {
        fn explore_buildings(
            ref self: ContractState, mut from_structure_id: ID, for_village: bool, for_village_direction: Direction,
        ) {
            let mut world: WorldStorage = self.world(DEFAULT_NS());
            // assert_caller_is_admin(world);
            ...
            for i in 0..18_u32 {
                ...
                let mut building_coord = buildings_center
                    .neighbor_after_distance(building_first_direction, building_direction_count.into());
                BuildingImpl::create(
                    ref world, structure_id, structure.category, structure.coord(), building_category, building_coord,
                );
                ...
                while building_direction_count > 1 {
                    let next_coord = building_coord
                        .neighbor_after_distance(building_second_direction, building_direction_count.into() - 1);
                    BuildingImpl::create(
                        ref world, structure_id, structure.category, structure.coord(), building_category, next_coord,
                    );
                    if i < buildings.len() {
                        ProductionStrategyImpl::burn_resource_for_resource_production(
...
pub mod cheat_map_systems_3 {
    ...
    use s1_eternum::models::config::{CombatConfigImpl, TroopLimitConfig, TroopStaminaConfig, WorldConfigUtilImpl};
    use s1_eternum::models::config::{MapConfig, TickImpl};
    use s1_eternum::models::map::Tile;
    use s1_eternum::models::position::{Coord, CoordImpl, Direction};
    use s1_eternum::models::resource::arrivals::{ResourceArrivalImpl};
    use s1_eternum::models::resource::production::building::{BuildingImpl};
    ...
    #[abi(embed_v0)]
    impl CheatMapSystems3Impl of super::ICheatMapSystems3<ContractState> {
        fn explore_mine_circles(ref self: ContractState, from_structure_id: ID, distance: u8) {
            let mut world: WorldStorage = self.world(DEFAULT_NS());
            // assert_caller_is_admin(world);
