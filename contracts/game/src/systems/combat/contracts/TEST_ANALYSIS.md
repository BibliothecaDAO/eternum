...
|------------|------------------------|----------------------|-----------------------|----------------------|
| guard_* | Yes (direct call) | No | Yes (auth check) | Yes (auth check) |
...
However, this would increase total CI time due to compilation overhead per job.
Only recommended if test isolation is needed or total test count grows significantly.
