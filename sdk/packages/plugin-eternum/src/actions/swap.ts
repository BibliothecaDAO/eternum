...
import { executeSwap as executeAvnuSwap, fetchQuotes, QuoteRequest } from "@avnu/avnu-sdk";
...
export const executeSwap: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      // Execute swap
      const swapResult = await executeAvnuSwap(getStarknetAccount(runtime), quote[0], {
        slippage: 0.05, // 5% slippage
        executeApprove: true,
      });

      elizaLogger.log("Swap completed successfully! tx: " + swapResult.transactionHash);
      callback?.({
