...
export default {
  ...
  handler: async (
    ...
    try {
      ...
      const amountInteger = Math.floor(Number(content.amount) * Math.pow(10, Number(decimals)));
      const amountWei = BigInt(amountInteger.toString());
      const transferCall = erc20Token.transferCall(content.recipient, amountWei);

      elizaLogger.success("Transferring", amountWei, "of", content.tokenAddress, "to", content.recipient);
