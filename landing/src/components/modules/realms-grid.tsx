  ...
  return (
    ...
    <AnimatedGrid
      ...
      renderItem={(realm, index) =>{
        const isSelected = isNftSelected && realm?.tokenMetadata.tokenId ? isNftSelected(
          realm?.tokenMetadata.tokenId,
          realm?.tokenMetadata.contractAddress,
        ) : false
        return( <RealmCard toggleNftSelection={toggleNftSelection} key={`${realm?.tokenMetadata.tokenId}-${index}`} isSelected={isSelected} realm={realm} />)}}
    />}</>
  );
};
