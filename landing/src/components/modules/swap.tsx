...
import { calculateDonkeysNeeded, getTotalResourceWeight } from "@/utils";
...
export const Swap = () => {
  ...
  const velordsFeeOnDeposit = useMemo(() => formatFee(EternumGlobalConfig.bridge.velords_fee_on_dpt_percent * Number(fromAmount) / feeDenominator), [fromAmount]);
  const seasonPoolFeeOnDeposit = useMemo(() => formatFee(EternumGlobalConfig.bridge.season_pool_fee_on_dpt_percent * Number(fromAmount) / feeDenominator), [fromAmount]);
  const clientFeeOnDeposit = useMemo(() => formatFee(EternumGlobalConfig.bridge.client_fee_on_dpt_percent * Number(fromAmount) / feeDenominator), [fromAmount]);
  const bankFeeOnDeposit = useMemo(() => formatFee(EternumGlobalConfig.bridge.max_bank_fee_dpt_percent * Number(fromAmount) / feeDenominator), [fromAmount]);

  const totalFeeOnDeposit = useMemo(() => formatFee(Number(velordsFeeOnDeposit) + Number(seasonPoolFeeOnDeposit) + Number(clientFeeOnDeposit) + Number(bankFeeOnDeposit)), [velordsFeeOnDeposit, seasonPoolFeeOnDeposit, clientFeeOnDeposit, bankFeeOnDeposit]);
  
  
  const velordsFeeOnWithdrawal = useMemo(() => formatFee(EternumGlobalConfig.bridge.velords_fee_on_wtdr_percent * Number(fromAmount) / feeDenominator), [fromAmount]);
  const seasonPoolFeeOnWithdrawal = useMemo(() => formatFee(EternumGlobalConfig.bridge.season_pool_fee_on_wtdr_percent * Number(fromAmount) / feeDenominator), [fromAmount]);
  const clientFeeOnWithdrawal = useMemo(() => formatFee(EternumGlobalConfig.bridge.client_fee_on_wtdr_percent * Number(fromAmount) / feeDenominator), [fromAmount]);
  const bankFeeOnWithdrawal = useMemo(() => formatFee(EternumGlobalConfig.bridge.max_bank_fee_wtdr_percent * Number(fromAmount) / feeDenominator), [fromAmount]);

  const totalFeeOnWithdrawal = useMemo(() => formatFee(Number(velordsFeeOnWithdrawal) + Number(seasonPoolFeeOnWithdrawal) + Number(clientFeeOnWithdrawal) + Number(bankFeeOnWithdrawal)), [velordsFeeOnWithdrawal, seasonPoolFeeOnWithdrawal, clientFeeOnWithdrawal, bankFeeOnWithdrawal]);
  ...
  return (
    <div className="w-96 flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex justify-between text-xs">
          <div>Season Pool Fees ({EternumGlobalConfig.bridge.season_pool_fee_on_dpt_percent * 100 / feeDenominator}%)</div>
