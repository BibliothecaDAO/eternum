...
export const IncomingOrder = ({ order, ...props }: IncomingOrderProps) => {
  ...
  let arrivalTime = getComponentValue(
    ArrivalTime,
    Utils.getEntityIdFromKeys([BigInt(order.orderId)]),
  );
  ...
  const startPosition = getComponentValue(
    Position,
    Utils.getEntityIdFromKeys([BigInt(order.counterpartyOrderId)]),
  );
  const startRealmId =
    startPosition &&
    getRealmIdByPosition({ x: startPosition.x, y: startPosition.y });
