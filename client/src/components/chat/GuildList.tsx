import Channel, { ChannelType } from "../../elements/Channel";
import { CreateGuildComponent } from "../CreateGuildComponent";

interface GuildListProps {
  guildList: ChannelType[];
  handleChat: any;
}

const GuildList = (props: GuildListProps) => {
  const { guildList, handleChat } = props;
  ...
  return (
    ...
    <div className="relative flex flex-col space-y-2 px-2 mb-2 ">
      ...
      {guildList.map((guild, index) => (
        <Channel key={index} channel={guild} handleChat={handleChat} />
      ))}
