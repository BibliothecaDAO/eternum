...
const RealmsPositions = () => {
  ...
  useEffect(() => {
    bufferGeometry.setAttribute("position", new THREE.BufferAttribute(calculatedPositions, 3));

    bufferGeometry.setAttribute("color", new THREE.BufferAttribute(calculatedColors, 3));
  }, []);
  ...
  const clickHandler = (e: any) => {
    ...
    setCameraPosition(
      new THREE.Vector3(point.x + (50 * Math.random() < 1 ? 1 : -1), 35, point.z + 50 * Math.random() < 1 ? 1 : -1),
    );
  };
  ...
  return (
    <>
      <points
        ...
        material={realmsMaterial}
        position={[realmsPosition.x, realmsPosition.y, realmsPosition.z]}
        rotation={[-Math.PI / 2, Math.PI, 0]}
        scale={[realmsScale, realmsScale, realmsScale]}
      />
    </>
  );
};
