...
export const RealmBadge = ({ active, realm, className, ...props }: RealmBadgeProps) => {
    ...
    return <div
        ...
        className={clsx('flex h-8 p-2 w-auto transition-all duration-200 items-center text-xs text-white border rounded-[10px] cursor-pointer border-gold bg-brown whitespace-nowrap',
        ...
        {...props}
    >
        <OrderIcon order={realm.order} size="xxs" color={hoveredOrActive ? 'white' : undefined} className='mr-1' />
        {name}
    </div>
};