...
export const NumberInput = ({ value, onChange, className, step = 1, max, min = 0 }: NumberInputProps) => (
    <div className={clsx('flex items-center border rounded-lg w-22 h-7 border-gold', className)}>
        ...
        <input type='number' min={min} className=' w-14 text-xs appearance-none !outline-none h-full text-center bg-transparent text-light-pink' value={value} onChange={(e) => onChange(parseInt(e.target.value))} />
