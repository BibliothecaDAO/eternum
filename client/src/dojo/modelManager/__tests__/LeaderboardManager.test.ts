...
import {
  generateMockCoOwnersChangeEvent,
  generateMockHyperstructureFinishedEvent,
  HYPERSTRUCTURE_ENTITY_ID,
  mockContributions,
  mockSingleContribution,
  OWNER_1_ADDRESS,
  OWNER_1_SHARES,
  OWNER_2_ADDRESS,
  OWNER_2_SHARES,
  TIMESTAMP,
} from "./__LeaderboardManagerMock__";
...
describe("processHyperstructureFinishedEventData", () => {
  ...
  it("should produce a valid map for 2 events processed", () => {
    ...
    expect(owner1Points2).toBe(5);
    expect(owner2Points2).toBeDefined();
    expect(owner2Points2).toBe(5);

    expect(owner1Points! + owner2Points!).toBe(HYPERSTRUCTURE_POINTS_ON_COMPLETION);
    expect(owner1Points2! + owner2Points2!).toBe(HYPERSTRUCTURE_POINTS_ON_COMPLETION);

    expect(owner1Points! + owner1Points2!).toBe(5 + 5);
    expect(owner2Points! + owner2Points2!).toBe(5 + 5);
  });
});
