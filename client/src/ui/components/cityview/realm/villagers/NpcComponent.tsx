...
export const NpcComponent = ({ entityId, realm }: NpcComponentProps) => {
  const {
    setup: {
      components: { Npc },
  ...
  const npc = useComponentValue(Npc, getEntityIdFromKeys([BigInt(realmEntityId), BigInt(entityId)]));
  ...
  const timeLeftToSpawn = useMemo(() => {
    ...
    // }
    return undefined;
  }, [npc, nextBlockTimestamp]);

  // TODO-JAM: get some villager sounds
