export default class GptInterface {
  private OPENAI_API_KEY = import.meta.env.VITE_OPENAI_API_KEY;
  ...
  GreetingsRole(npc) {
    return `The lord of your realm is coming to say hello to you.
  ...
  HarvestRole(npc) {
    return `The lord of your realm has ordered you to ${this.harvest_descriptions[npc.role]}.
	Your hunger state is ${this.hunger[npc.mood.hunger]},
	your happiness state is ${this.happiness[npc.mood.happiness]}
	and your belligerent state is ${this.belligerent[npc.mood.belligerent]}.
	Use these states and the current context of your realm as the characters current feelings without mentioning them directly.
  ...
  BuildRole(npc) {
    return `The lord has invested in infrastructure and tools for the ${this.roleLocation[npc.role]}. 
  ...
  VisitRole(npc) {
    return `The lord of your realm is visiting the ${this.roleLocation[npc.role]}. 
	Your hunger state is ${this.hunger[npc.mood.hunger]},
	your happiness state is ${this.happiness[npc.mood.happiness]}
	and your belligerent state is ${this.belligerent[npc.mood.belligerent]}.
	Use these states and the current context of your realm as the characters current feelings without mentioning them directly.
  ...
  async generatePrompt(promptSystem, promptUser) {
    ...
    try {
      const response = fetch(this.OPENAI_URL, requestOptions);
      return response;
  ...
  generateGreetingPrompts(npc) {
    ...
    const promptSystem = `Imagine yourself as a ${this.sex[npc.sex]} ${this.role[npc.role]} in the medieval realm of ${
      this.realmState.name
    			  ...
    			  Your realm's military defense is ${this.realmState.defense} and the general sentiment of the populace is ${
      this.realmState.happiness
    }.
    			  Keep the answer under 50 words.`;
    return this.generatePrompt(promptSystem, this.GreetingsRole(npc));
  }
    ...
    for (const npc of npcs) {
      if (npc.role == harvestRole) {
        const promptSystem = `Imagine yourself as a ${this.sex[npc.sex]} ${
          this.role[npc.role]
        } in the medieval realm of ${this.realmState.name}.
                    In this realm, your society is deeply involved in mining various minerals,
                    sustaining its population through agriculture and fishing, establishing prosperous trade routes,
                    and maintaining a robust military defense against potential threats from foreign realms.
