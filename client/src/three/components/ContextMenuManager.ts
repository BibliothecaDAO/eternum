...
export class ContextMenuManager {
	...
	private showContextMenuForHexagon(event: MouseEvent) {
		...
		if (intersects.length > 0) {
			const hexCoords = this.getHexagonCoordinates(intersects[0].object as THREE.InstancedMesh, intersects[0].instanceId!);
	...
	private getHexagonCoordinates(instancedMesh: THREE.InstancedMesh, instanceId: number): { row: number; col: number } {
	...
	private onMouseUp(event: MouseEvent) {
		if (event.button === 2) {
			...
			const clickDistance = Math.sqrt(
				Math.pow(event.clientX - this.rightClickStartPosition.x, 2) + Math.pow(event.clientY - this.rightClickStartPosition.y, 2)
			);
