...
const TOTAL_EFFECTIVE_CONTRIBUTION = 8209.10;

// fix ?
const ResourceMultipliers: { [key in ResourcesIds]?: number } = {
  [ResourcesIds.Wood]: 1.00,
  [ResourcesIds.Stone]: 1.27,
  [ResourcesIds.Coal]: 1.31,
  [ResourcesIds.Copper]: 1.90,
...
export const useComputePointsLeaderboards = () => {
  ...
  const updatePointsLeaderboard = ((event: any): PlayerPointsLeaderboardInterface[] => {
    ...
    contributionsToHyperstructure().forEach((contribution) => {
      ...
      tempPlayerPointsLeaderboard[index].totalPoints += computeContributionPoints(totalHyperstructurePoints, contribution.amount, BigInt(contribution.resource_type));
    })
