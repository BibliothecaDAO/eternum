import { calculateLordsShare, calculatePlayerSharePercentage, Player, StructureType } from "@bibliothecadao/eternum";
import { getComponentValue, Has, HasValue, runQuery } from "@dojoengine/recs";
import { shortString } from "starknet";
import { formatEther } from "viem";
import { useDojo, useEntitiesUtils, useLeaderBoardStore } from "../";
import { usePrizePool } from "./use-rewards";

export const useGetAllPlayers = (env: { viteLordsAddress: string }) => {
  const dojo = useDojo();
  ...
  const prizePool = usePrizePool(env);
