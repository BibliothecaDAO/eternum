...
export const useMarketHistory = (market: MarketClass, refreshKey = 0) => {
  ...
  useEffect(() => {
    const initAsync = async () => {
      ...
      const rawDenominators = items.flatMap((i) => {
      ...
      const denominatorsByTimestamp = new Map<string, VaultDenominatorEvent>();
      for (const event of rawDenominators) {
        denominatorsByTimestamp.set(String(event.timestamp), event);
      }

      const denominators = Array.from(denominatorsByTimestamp.values()).sort(
