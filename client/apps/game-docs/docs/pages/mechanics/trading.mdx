...
import { CapacityConfigCategory } from "@/utils/types";
...
> ⚠️ **Important**: Even though liquidity is tracked at the account level, you must maintain ownership of at least one
> realm to manage your liquidity positions

## Transport <img src="/images/resources/25.png" alt="Donkey" width="200" className="float-right" />

Donkeys are essential for all resource transfers. Each donkey can carry up to
**{(ETERNUM_CONFIG().carryCapacityGram[CapacityConfigCategory.Donkey] / 1000).toLocaleString()} kg**, and transport time
depends on the distance to its destination. It's important to consider transport costs in your trade calculations.
...
- **Sending Resource**s: When you want to send resources (like **$LORDS**) to another realm, the bridge calculates the
  ...
  the distance and is calculated as {ETERNUM_CONFIG().speed.donkey} seconds per kilometer.
...
## 🏦 Central Bank <img src="/images/buildings/construction/bank.png" alt="Bank" width="300" className="float-right" />
...
> ⚠️ **Note:** When the Bank is being attacked, the AMM will be **locked** until the battle has ended.
