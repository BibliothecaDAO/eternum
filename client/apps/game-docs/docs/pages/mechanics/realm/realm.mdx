...
import { CapacityConfigCategory, RealmLevels } from "@/utils/types";
...
> - **Population**: {ETERNUM_CONFIG().populationCapacity.basePopulation}
> - **Military Capacity**:
>   - 3 attacking armies
>   - 1 defending army
> - **Resource Storage**:
>   {formatNumberWithCommas((ETERNUM_CONFIG().carryCapacityGram[CapacityConfigCategory.Storehouse]))} kg
>
...
- <span>
    {(ETERNUM_CONFIG().tick.armiesTickIntervalInSeconds / 3600) * ETERNUM_CONFIG().battle.graceTickCount} hour immunity
    from attacks and from attacking others
  </span>
