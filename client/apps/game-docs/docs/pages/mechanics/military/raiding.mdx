...
Raidable resources are stolen based on rarity, with the most valuable taken first. Rarer resources are prioritized
during raids:

<RaidableResources />

<div style={resource.noteStyle}>Note: Tokens such as $LORDS and $STRK cannot be stolen through raiding.</div>
