...
- **One item per slot** - A cosmetic NFT can only be equipped to a slot on a single Realm. If Lords wish to have
  multiple Realms with the same cosmetic item, they will require multiple copies of that NFT. _Note: Initial versions of
  Blitz will only have 14 slots available that apply to all 3 starting Realms of a player. Cosmetics will apply to single 
  Realms in both Blitz and Eternum game modes from Eternum Season 2 onwards._
