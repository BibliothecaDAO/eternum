...
Initialize runtime directories and config:

```bash
axis init
```

This creates `~/.eternum-agent/.env` with a config template. Edit it to set your API key:

```bash
# Open the config file and set ANTHROPIC_API_KEY (or OPENAI_API_KEY)
nano ~/.eternum-agent/.env
```
...
1. Load config from `~/.eternum-agent/.env`
2. Discover active worlds
3. Let you select a world (unless `SLOT_NAME` is set)
4. Open browser-based Cartridge session approval
