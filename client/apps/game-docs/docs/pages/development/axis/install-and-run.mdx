...
# 2. Auth with password (no browser needed)
...
Password auth registers a Cartridge Controller session with paymaster support — no browser, no QR code, no ports to
open. See [Worlds and Auth](/development/axis/worlds-and-auth) for alternative auth methods (QR code, redirect URL).
