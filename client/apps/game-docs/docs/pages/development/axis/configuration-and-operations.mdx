...
1. **`~/.eternum-agent/.env`** — global config (API keys, preferences). Created by `axis init`.
...
Values from higher-priority sources override lower ones. Shell env vars are never overwritten by file values.
