...
3. **Shell environment** — `export ANTHROPIC_API_KEY=...` always wins over file values.

Values from higher-priority sources override lower ones. Shell env vars are never overwritten by file values. All
runtime directories (`~/.eternum-agent/data/`, `~/.eternum-agent/.cartridge/`) are auto-created on first run — no manual
`axis init` required.
