...
recently moved away from “orders” in favor of preset-driven allocations.
...
- **Per-resource percentages**: Instead of one-off jobs, each resource has sliders that describe how much of its inputs
  you are willing to burn through complex (resource→resource) or simple (labor→resource) recipes.
...
- **Presets**: “Labor”, “Resource”, “Custom”, and “Idle” presets instantly distribute percentages across every resource
...
> ⚠️ The automation loop runs inside your browser. Keep the Blitz tab in the foreground (or at least open) whenever you
...
2. **Resource scan**: Any resource with buildings or active production is auto-added (unless it’s blocked) so its inputs
   are tracked even if you never touched the sliders.
3. **Budgeting**: For each resource the system computes a “budget” equal to 90% of its simulated balance at the current
   tick. Percentages merely describe how much of that budget to dedicate to complex vs simple recipes.
4. **Plan generation**:
   - Complex recipes reserve inputs based on your percentage cap and the recipe’s per-cycle requirement.
...
6. **Feedback loop**: The store records how many cycles ran, total inputs consumed, outputs produced, and any skipped
   resources so the UI can show “last run” stats.
...
- **Donkeys** default to 10% resource→resource and 0% labor→resource. The slider enforces these caps so you always have
...
3. **Adjust percentages**:
   - `Resource → Resource`: How much of that resource’s complex inputs you are willing to burn per loop.
   - `Labor → Resource`: How much labor-based production to run. (Disabled for Donkeys and military-only recipes.)
...
1. **Complex (Resource → Resource)**:
   - Burns existing resources to craft another output.
   - Uses the `Resource → Resource` slider for each resource entry.
   - Example: Converting Wood + Stone into Tools.
2. **Simple (Labor → Resource)**:
   - Spends Labor to produce a resource.
   - Uses the `Labor → Resource` slider.
   - Example: Spending Labor to harvest Food.

`Resource → Labor` conversions and any recipe that would output Wheat or Labor are blocked from automation entirely.
Those actions must be run manually from the production UI.

## Allocation & Execution Rules

- **Percentages replace orders**: There are no longer bespoke “produce once/maintain” orders. Every loop recomputes
...
- **Interval-based**: Each entry stores a source, destination, resource list, and interval. Intervals are clamped
  between 1 and 30 minutes (default 5). The runner wakes roughly once per second to check whether an entry’s `nextRunAt`
  has elapsed.
- **Immediate send + schedule**: When you enable “Repeat” in the transfer panel, Blitz sends the payload once (if
  possible) and also creates one entry per destination. The immediate send time is recorded as `lastRunAt`, and the
  first automated run occurs after the selected interval.
...
- **Safety checks**: Military resources can only move Realm ↔ Realm. Fragment mines can only send Donkeys/Essence. The
...
1. **Choose a source**: Realms, Villages, and Fragment Mines you own appear in the “Transfer” panel. Fragment Mines are
...
4. **Set frequency**:
   ...
   - `Repeat` executes immediately and schedules future runs at the chosen 1‑30 minute interval (slider defaults to 5
     minutes).
...
- Inspect per-entry stats, including the configured interval and a “last run” timestamp (immediate runs show as “just
  now”).
- Pause/resume individual entries; pausing clears `nextRunAt` until you resume.
- Use the row-level actions to `Run now`, `Pause/Resume`, or `Delete` a specific route without touching the rest.
- Bulk actions (pause/resume/run active now) operate on the currently filtered list so you can target specific routes.
  Manual runs obey the same donkey and resource checks as the automated loop.

### Transfer Requirements & Safeguards

- Sources and destinations must be structures you own. The UI only lists owned entities and the runner double-checks IDs
  before calling `send_resources_multiple`.
...
- **Loop Cadence**: Production automation evaluates every ~60 seconds. The transfer runner polls roughly once per second
  but only fires entries whose 1‑30 minute interval has elapsed.
...
- **Interval Limits**: Every scheduled entry enforces a 1‑30 minute interval. Values below 1 minute are raised to 1 and
  anything above 30 minutes is clamped down by the UI slider.
- **Resource & Donkey Checks**: Each run clamps requested amounts to the source’s current balance and donkey capacity.
  If either requirement fails, the entry is skipped and rescheduled.
- **Payload Restrictions**: Military resources require Realm ↔ Realm routes. Fragment mines may only ship Donkeys and
  Essence.
