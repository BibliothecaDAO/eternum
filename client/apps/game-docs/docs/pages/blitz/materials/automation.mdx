...
- **Interval-based**: Each entry stores a source, destination, resource list, and interval. Intervals are clamped between
  1 and 30 minutes (default 5). The runner wakes roughly once per second to check whether an entry’s `nextRunAt` has
  elapsed.
- **Immediate send + schedule**: When you enable “Repeat” in the transfer panel, Blitz sends the payload once (if
  possible) and also creates one entry per destination. The immediate send time is recorded as `lastRunAt`, and the first
  automated run occurs after the selected interval.
...
4. **Set frequency**:
   ...
   - `Repeat` executes immediately and schedules future runs at the chosen 1‑30 minute interval (slider defaults to
     5 minutes).
...
- **Resource & Donkey Checks**: Each run clamps requested amounts to the source’s current balance and donkey capacity. If
