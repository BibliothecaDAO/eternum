...
There are currently nine relic types, each with two levels. A level 2 relic is simply a more powerful version of
the level 1 counterpart. Every relic type has a distinct effect, which determines whether it can be activated by 
a Realm or an army. To activate a relic and gain the desired effect, the army or Realm must pay an amount of Essence.
These details, as well as the discovery chance of each relic in a Relic Crate, are defined in the table below:
