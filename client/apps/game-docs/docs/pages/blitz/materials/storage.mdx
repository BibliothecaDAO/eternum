...
## Weight and Storehouses

### Material Weights

Materials have different weights that affect storage:

<span style={{ color: "red" }}>DRAFTING NOTE: Review and add essence.</span>

<WeightTable />

### Storehouses <img src="/images/buildings/construction/storehouse.png" alt="Storehouse" width="300" className="float-right"/>

Each storehouse adds {formatNumberWithCommas(ETERNUM_CONFIG().carryCapacityGram[CapacityConfig.Storehouse]/1000 )}kg of storage capacity.
Lords must be careful not to exceed the storage capacity of their Realms, as any produced materials that cannot be stored will be permanently lost.
...
| Essence Rift   | {formatNumberWithCommas(ETERNUM_CONFIG().carryCapacityGram[CapacityConfig.EssenceRiftStructure]/1000)} kg    |
| Camp           | {formatNumberWithCommas(ETERNUM_CONFIG().carryCapacityGram[CapacityConfig.CampStructure]/1000)} kg           |