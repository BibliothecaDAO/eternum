...
Troops can also be moved between player-owned structures using donkeys, but to dominate your enemies and ensure
security of your lands, troops must be deployed to an army on the world map. Once troops are deployed, they cannot be
taken back into the inventory. Their mobility is then governed by stamina, limiting the distance they can travel and
the number of actions they can perform. Traveling between hexes or initiating an attack expends valuable stamina
points, which are partially replenished each Eternum Day.
