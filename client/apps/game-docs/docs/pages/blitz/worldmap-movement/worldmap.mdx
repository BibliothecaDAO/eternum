import { HyperstructureRings } from "@/components/blitz-components/HyperstructureRings";
...
bounds of the Hyperstructure rings. While the order in which players are placed is randomized, the positioning is
...
In standard Blitz, each participating Lord receives three Realms positioned three hexes apart in a triangle formation.
Those Realm tiles are aligned to Hyperstructure scoring checks at an eight-hex offset. In single-realm mode, each Lord's
single centered Realm aligns at a ten-hex offset instead. More details of the positioning system are available in the
...
activate the crate will receive **three** Relics directly into their inventory and **1,000 victory points** for their
Lord, so be sure to secure them before your enemies do!
