...
The amount of stamina consumed when moving into an explored hex depends on both the biome of that hex and the type of troops
...
Exploration rewards are automatically sent back to the home Realm of the exploring army, so armies aren't limited by their
carry capacity in Blitz. However, the mist is a scary place and troops don't like to delve into it alone, so a minimum of
10 troops are required in an army to explore a hex.
...
on the worldmap using left-click, then right-click on the target and select the Transfer Troops tab. Troops
