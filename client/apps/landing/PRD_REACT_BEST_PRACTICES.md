...
The landing app has correctness and performance risks in high-traffic routes (`/`, `/$collection`, `/trade/$collection`,
`/mint`) that increase bundle cost, trigger unnecessary rerenders/refetches, and can produce incorrect marketplace data
in some flows.

This remediation effort defines a focused, measurable upgrade to React/TanStack-query patterns aligned with Vercel React
best practices (bundle, async/data, rerender optimization).
