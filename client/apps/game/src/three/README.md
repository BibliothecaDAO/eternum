...
- On chunk change `performChunkSwitch()`:
  1. syncs projected tiles and prepares bounds, then commits `currentChunk` through one finalize-helper callback path
     before manager fanout while registering/unregistering chunk bounds with the `CentralizedVisibilityManager`.
  2. Reads tiles for the new render window from the session-owned spatial projection without issuing camera-driven Torii
     queries.
...
- Chunk preparation reads bucketed bounds from `WorldSpatialProjection`; camera movement selects already-synced data and
  does not own current game state.
