...
export default class WorldmapScene extends WarpTravel {
  ...
  private readonly worldmapZoomDistance = resolveWorldmapCameraViewProfile(LOCKED_WORLDMAP_CAMERA_VIEW).distance;
  private readonly worldmapMinZoomDistance = this.worldmapZoomDistance;
  private readonly worldmapMaxZoomDistance = this.worldmapZoomDistance;
  ...
  private lastPublishedStableCameraView = LOCKED_WORLDMAP_CAMERA_VIEW;
  ...
  private resolveCameraTargetState() {
    ...
    return {
      targetHex: { col: Number(contractHex.x), row: Number(contractHex.y) },
      distance: Math.round(this.controls.object.position.distanceTo(this.controls.target) * 100) / 100,
    };
  }
  private updateCameraTargetHex = () => {
    const cameraTargetState = this.resolveCameraTargetState();
    const state = useUIStore.getState();
    const currentHex = state.cameraTargetHex;
    const hexChanged =
      !currentHex ||
      currentHex.col !== cameraTargetState.targetHex.col ||
      currentHex.row !== cameraTargetState.targetHex.row;
    const distanceChanged =
      state.cameraDistance === null || Math.abs(state.cameraDistance - cameraTargetState.distance) > 0.01;

    if (!hexChanged && !distanceChanged) return;

    const nextState: { cameraTargetHex?: typeof cameraTargetState.targetHex; cameraDistance?: number } = {};
    if (hexChanged) nextState.cameraTargetHex = cameraTargetState.targetHex;
    if (distanceChanged) nextState.cameraDistance = cameraTargetState.distance;
  ...
  private handleWorldmapControlsChange = () => {
    if (this.sceneManager.getCurrentScene() !== SceneName.WorldMap) return;
    const previousTargetHex = useUIStore.getState().cameraTargetHex;
    const nextTargetState = this.resolveCameraTargetState();
    this.updateCameraTargetHexThrottled?.();

    const refreshPlan = resolveControlsChangeChunkRefreshPlan({
      previousTargetHex: previousTargetHex ?? null,
      nextTargetHex: nextTargetState.targetHex,
    });
  ...
  constructor(
    ...
    this.currentCameraView = LOCKED_WORLDMAP_CAMERA_VIEW;
    this.targetCameraView = LOCKED_WORLDMAP_CAMERA_VIEW;
  ...
  private setupCameraZoomHandler(): void {
    this.syncLockedWorldmapCameraView();
  }

  public override changeCameraView(position: CameraView) {
    const lockedView = resolveLockedWorldmapCameraView(position);
    this.targetCameraView = lockedView;
    if (!this.isCameraDistanceOutOfSync(lockedView)) {
      this.syncLockedWorldmapCameraView();
      return;
    }

    this.alignWorldmapCameraToBand(lockedView);
    this.syncLockedWorldmapCameraView();
  }
  ...
  private alignInitialWorldmapCameraView(): void {
    this.alignWorldmapCameraToBand(LOCKED_WORLDMAP_CAMERA_VIEW);
    this.syncLockedWorldmapCameraView();
  }
  ...
  private configureWarpTravelSetupStart(): void {
    ...
    this.controls.minDistance = this.worldmapMinZoomDistance;
  ...
  private resetZoomHardeningRuntimeState(): void {
    ...
    this.syncLockedWorldmapCameraView();
    this.lastPublishedStableCameraView = LOCKED_WORLDMAP_CAMERA_VIEW;
  ...
  protected override syncResolvedCameraViewFromDistance(_distance: number): void {
    this.syncLockedWorldmapCameraView();
  }

  private syncLockedWorldmapCameraView(): void {
    this.currentCameraView = LOCKED_WORLDMAP_CAMERA_VIEW;
    this.targetCameraView = LOCKED_WORLDMAP_CAMERA_VIEW;

    if (this.lastPublishedStableCameraView === LOCKED_WORLDMAP_CAMERA_VIEW) {
      return;
    }

    this.lastPublishedStableCameraView = LOCKED_WORLDMAP_CAMERA_VIEW;
    this.worldmapCameraViewListeners.forEach((listener) => listener(LOCKED_WORLDMAP_CAMERA_VIEW));
  }
