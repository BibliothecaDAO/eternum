...
class PerformanceMonitorImpl {
  ...
  public logSummary(matrixPoolStats?: Parameters<typeof this.generateReport>[0]): void {
    ...
    console.log(`Frame Time: avg=${report.frameTime.avg}ms, min=${report.frameTime.min}ms, max=${report.frameTime.max}ms`);
    ...
    console.log(`Matrix Pool: ${report.memory.matrixPool.inUse} in use, ${report.memory.matrixPool.totalAllocated} allocated (${report.memory.matrixPool.memoryMB}MB)`);
