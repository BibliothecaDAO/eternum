...
export function buildArmyLabelDataKey(army: ArmyLabelContentFields): string {
  return `${army.troopCount}-${army.currentStamina}-${Math.round((army.displayStaminaRatio ?? 0) * 10000)}-${army.battleTimerLeft ?? 0}-${army.isMine}-${army.owner.ownerName}-${army.attackedFromDegrees ?? ""}-${army.attackedTowardDegrees ?? ""}`;
}
