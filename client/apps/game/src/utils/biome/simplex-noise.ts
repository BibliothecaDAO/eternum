...
export function noise(v: Vec3): Fixed {
  ...
  let _p2 = permute(
    Vec4.new(_p1.x.add(i.y).add(zero), _p1.y.add(i.y).add(i1.y), _p1.z.add(i.y).add(i2.y), _p1.w.add(i.y).add(one)),
  );
  let p = permute(
    Vec4.new(_p2.x.add(i.x).add(zero), _p2.y.add(i.x).add(i1.x), _p2.z.add(i.x).add(i2.x), _p2.w.add(i.x).add(one)),
  );
  ...
  let sh = Vec4.new(step(h.x, zero).neg(), step(h.y, zero).neg(), step(h.z, zero).neg(), step(h.w, zero).neg());
