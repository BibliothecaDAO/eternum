...
export const CommonAutomations: React.FC<CommonAutomationsProps> = ({
  ...
  const renderOrderPreview = (order: Omit<AutomationOrder, "id" | "producedAmount">, key?: React.Key) => {
    ...
    const displayedIconGroups =
      resourceIconGroups.length === 0 && order.resourceToUse !== undefined
        ? [[order.resourceToUse]]
        : resourceIconGroups;
    ...
    <div className="mb-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {presets.map((preset) => {
          ...
          const previewOrder = preset.previewOrder ?? orders[0];
          const isDisabled = !preset.available || isRealmPaused || orders.length === 0;
          const disabledReason = preset.unavailableReason || (isRealmPaused ? "Realm is paused" : undefined);
          const addLabel = orders.length > 1 ? `Adds ${orders.length} automations` : "Ready to add";
            ...
            <div key={preset.id} className="border border-gold/20 rounded-lg p-3 bg-black/20 flex flex-col gap-2">
              ...
              {orders.length > 1 ? (
                <div className="flex flex-col gap-2">
                  {orders.map((order, idx) => renderOrderPreview(order, `${preset.id}-order-${idx}`))}
                </div>
              ) : (
                previewOrder && renderOrderPreview(previewOrder)
              )}
              <div className="flex items-center justify-between mt-auto">
                {isDisabled && disabledReason ? (
                  <span className="text-[11px] text-gold/60">{disabledReason}</span>
                ) : (
                  <span className="text-[11px] text-gold/60">{addLabel}</span>
                )}
                <Button
                  type="button"
                  variant="outline"
