...
export const StructureCard = memo(({
  ...
  const accentGlow = tone === "source" ? "shadow-[0_0_18px_rgba(16,185,129,0.25)]" : "shadow-[0_0_18px_rgba(56,189,248,0.25)]";
  ...
  const overflowTooltip = useMemo(() => {
    ...
    return preview.balanceOverflow
      .map((item) => `${item.label}: ${formatBalance(item.balance)}`)
      .join("\n");
  ...
  return (
    <div
      ...
      className={clsx(
        "group relative w-full rounded-xl border border-gold/15 bg-black/30 p-4 transition-all duration-200",
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {preview.meta.hasWonderBonus && (
            <span className="flex items-center gap-1 rounded-md bg-gold/15 px-2 py-1 text-[10px] font-semibold uppercase text-gold" title={`Wonder bonus active${preview.meta.wonderBonusPercent ? ` • +${preview.meta.wonderBonusPercent}%` : ""}`}>
              <Sparkles className="h-3 w-3" /> Wonder
            </span>
          )}
          {preview.meta.activeRelicCount > 0 && (
            <span className="rounded-md bg-purple-500/20 px-2 py-1 text-[10px] font-semibold uppercase text-purple-200" title={`${preview.meta.activeRelicCount} relic${preview.meta.activeRelicCount === 1 ? "" : "s"} reinforcing production`}>
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-4">
          {preview.resources.length === 0 && <span className="text-xs text-gold/60">Idle — no current throughput</span>}
            ...
            return (
              <div key={`${structure.entityId}-${resource.resourceId}`} className="flex flex-col items-center gap-1">
                <ProductionStatusBadge
                  ...
                  size="xs"
                />
                <span className="text-[11px] font-semibold text-emerald-200">{rate ?? "0/h"}</span>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col leading-tight text-xs text-gold/80">
                  ...
                  <span className="text-[10px] text-gold/60">{rate ?? "Stable"}</span>
                </div>
              </div>
            );
          })}
