...
export const CosmeticShowcase = ({ item }: CosmeticShowcaseProps) => {
  ...
  return (
    <section className="relative z-10 flex h-full flex-col justify-between gap-6">
      <div className="relative flex-1 overflow-hidden rounded-2xl border border-gold/20 bg-gradient-to-br from-gold/5 via-black/20 to-black/70 transition [cursor:grab] hover:border-gold/40 active:[cursor:grabbing] min-h-[320px]">
        {item ? (
          <div className="h-full w-full">
            <CosmeticModelViewer modelPath={item.modelPath} variant="showcase" autoRotate />
          </div>
        ) : (
          <div className="flex h-full items-center justify-center text-sm text-gold/60">
            Select a cosmetic to preview.
          </div>
        )}
        {item && (
          <div className="pointer-events-none absolute left-4 top-4 flex items-center gap-2 rounded-2xl border border-gold/20 bg-black/55 px-4 py-2 backdrop-blur">
            <div className="text-xs text-gold/70">
              <p className="font-semibold text-gold">Owned</p>
      ...
      <footer className="space-y-4 rounded-2xl border border-gold/20 bg-black/40 p-5 shadow-[inset_0_1px_0_rgba(255,215,128,0.04)]">
        <div className="space-y-2">
          <h3 className="text-2xl font-semibold text-gold">{item?.name ?? "Select a cosmetic"}</h3>
        ...
        <div className="flex flex-col gap-3">
          {slot && (
            <div className="flex flex-wrap items-center gap-2 text-xs text-gold/60">
              <span className="rounded-full border border-gold/20 bg-black/40 px-3 py-1 capitalize">
          ...
          <div className="flex items-center gap-2">
            ...
            {item?.tokenId && (
              <a
                ...
                className="flex h-10 items-center gap-2 rounded-lg border border-gold/20 bg-black/40 px-4 text-sm text-gold/70 transition hover:border-gold/50 hover:text-gold"
              >
        ...
        <div className="space-y-3 pt-2">
          <div>
            <h4 className="text-xs font-semibold uppercase tracking-wide text-gold/60">Attributes</h4>
            {attributes.length > 0 ? (
              <dl className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
                {attributes.map((attribute) => (
                  <div
                    key={`${attribute.trait_type}-${attribute.value}`}
                    className="rounded-xl border border-gold/10 bg-black/40 px-3 py-2"
                  >
                    <dt className="text-[0.65rem] uppercase tracking-wide text-gold/40">{attribute.trait_type}</dt>
                    <dd className="text-sm font-medium text-gold">{attribute.value}</dd>
                  </div>
                ))}
              </dl>
            ) : (
