...
export const CollectionProgress = ({ ownedCount, totalCount, isLoading = false }: CollectionProgressProps) => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-4 rounded-xl border border-white/10 bg-black/40 px-4 py-3">
  ...
  return (
    <div className="flex items-center gap-4 rounded-xl border border-white/10 bg-black/40 px-4 py-3">
      ...
      <div className="flex items-baseline gap-1 text-sm">
        ...
        <span className="ml-1 text-xs text-white/40">({percentage}%)</span>
      </div>
    </div>
  );
};
