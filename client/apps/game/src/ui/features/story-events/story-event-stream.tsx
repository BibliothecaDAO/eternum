...
const STORY_EVENT_THEMES: Record<StoryEventIcon, { accent: string; highlight: string; icon: ComponentType<{ className?: string }> }> = {
  realm: { accent: "from-fuchsia-500/80 via-fuchsia-400/30 to-transparent", highlight: "text-fuchsia-200", icon: Castle },
  building: { accent: "from-sky-400/80 via-sky-300/25 to-transparent", highlight: "text-sky-200", icon: Hammer },
  production: { accent: "from-emerald-500/80 via-emerald-300/25 to-transparent", highlight: "text-emerald-200", icon: Factory },
  ...
  scroll: { accent: "from-slate-500/80 via-slate-300/25 to-transparent", highlight: "text-slate-200", icon: ScrollText },
};
...
function StreamItem({ item }: StreamItemProps) {
  ...
  return (
    ...
    >
      <span className={cn("absolute inset-y-1 left-0 w-[2px] rounded-full opacity-60 bg-gradient-to-b", theme.accent)} aria-hidden />
      <div className="flex items-start gap-1.5">
        ...
        <div className="flex-1">
          ...
          {event.description && (
            <p className="mt-0.5 text-[10px] text-zinc-400/70 line-clamp-2">{event.description}</p>
          )}
        </div>
      </div>
    </motion.li>
