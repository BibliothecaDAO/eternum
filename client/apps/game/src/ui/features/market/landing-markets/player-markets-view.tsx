...
const StatCard = ({
  ...
  return (
    <Card className="group relative overflow-hidden border-gold/20 bg-gradient-to-br from-brown/65 to-brown/45 p-4 transition-all hover:border-gold/35 hover:from-brown/75 hover:to-brown/55">
      ...
      <div className="relative space-y-1">
        ...
        <div className={`flex items-center gap-2 text-xl font-bold ${trendColor || "text-lightest"}`}>
...
const PlayerMarketRow = ({ entry }: { entry: PlayerSummary["markets"][number] }) => {
  ...
  return (
    <div className="group grid grid-cols-1 gap-3 rounded-lg py-4 transition-all hover:bg-gold/5 md:grid-cols-[1fr_auto]">
      <div className="min-w-0 space-y-2">
        <HStack className="items-center gap-2">
          {isLinkable ? (
            <Link
              className="flex items-center gap-1.5 font-semibold leading-tight text-gold transition-colors hover:text-gold/80"
              to={href}
              title={market?.title}
            >
            ...
            <div className="font-semibold leading-tight text-gold">{market?.title || "Unknown market"}</div>
          )}
          {market ? <MarketStatusBadge market={market} /> : null}
        </HStack>
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">
          <span className="inline-flex items-center gap-1.5 text-gold/60">
            <span className="font-medium text-gold/60">Volume</span>
            <span className="font-semibold text-lightest">{formatNumber(entry.volume, 2)}</span>
            {market?.collateralToken && <TokenIcon token={market.collateralToken as RegisteredToken} size={14} />}
          </span>
          <span className="inline-flex items-center gap-1.5 text-gold/60">
            <span className="font-medium text-gold/60">Earned</span>
            <span className="font-semibold text-brilliance">{formatNumber(entry.earned, 2)}</span>
            {market?.collateralToken && <TokenIcon token={market.collateralToken as RegisteredToken} size={14} />}
          </span>
        </div>
      </div>
      <div className="flex items-center justify-end">
        {showClaim ? (
          <button
            type="button"
            className="flex items-center gap-2 rounded-lg border border-gold/35 bg-gradient-to-r from-gold/15 to-gold/10 px-4 py-2.5 text-sm font-semibold text-gold transition-all hover:border-gold/55 hover:from-gold/25 hover:to-gold/15"
            onClick={() => redeem()}
          ...
          <div
            className={`flex items-center gap-1.5 rounded-lg px-4 py-2.5 text-sm font-bold transition-all ${
              entry.pnl >= 0
                ? "bg-brilliance/10 text-brilliance ring-1 ring-brilliance/25"
...
export const PlayerMarketsView = ({ isLoading, onRefresh, hasWallet, summary }: PlayerMarketsViewProps) => {
      ...
      <div className="space-y-6" aria-busy aria-live="polite">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_item, idx) => (
            <div key={idx} className="h-24 animate-pulse rounded-xl bg-brown/55" />
          ))}
        </div>
        <div className="h-80 animate-pulse rounded-xl bg-brown/55" />
      </div>
    );
  }

  const collateralToken = summary.markets[0]?.market?.collateralToken as RegisteredToken | undefined;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="grid flex-1 grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          <StatCard
            ...
            trendColor={summary.pnl >= 0 ? "text-brilliance" : "text-danger"}
          />
      ...
      <Card className="overflow-hidden border-gold/20 bg-gradient-to-br from-brown/65 to-brown/45">
        <CardHeader className="border-b border-gold/10 px-5 py-4">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg font-bold text-gold">Your Markets</CardTitle>
        ...
        <CardContent className="p-0">
          {summary.markets.length === 0 ? (
            ...
            <ScrollArea className="max-h-[480px]">
              <div className="px-5">
                <VStack className="divide-y divide-gold/10">
