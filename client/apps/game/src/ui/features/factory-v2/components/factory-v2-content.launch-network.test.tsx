...
vi.mock("@/ui/components/switch-network-prompt", () => ({
  SwitchNetworkPrompt: ({ open }: { open: boolean }) => (open ? <div>Switch prompt</div> : null),
}));

vi.mock("@/ui/utils/network-switch", () => ({
  getChainLabel: vi.fn((chain: string) => chain),
...
describe("FactoryV2Content network handling", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(useAccount).mockReturnValue({
      chainId: "0x1",
      connector: { controller: {} as WalletChainControllerLike },
    } as unknown as ReturnType<typeof useAccount>);
    vi.mocked(resolveConnectedTxChainFromRuntime).mockReturnValue("slot");
  });
  ...
  it("launches immediately without opening the switch prompt when the wallet is on another network", async () => {
    ...
    expect(container.textContent).not.toContain("Switch prompt");
    expect(vi.mocked(switchWalletToChain)).not.toHaveBeenCalled();
  });

  it("still asks for a network switch before continue actions", async () => {
    const factory = buildFactoryState({
    ...
    expect(container.textContent).toContain("Switch prompt");
    expect(factory.continueSelectedRun).not.toHaveBeenCalled();
  });
});
