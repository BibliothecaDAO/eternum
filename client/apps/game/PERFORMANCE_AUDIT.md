...
**Date:** 2024-12-17 **Scope:** React/TypeScript performance analysis - N+1 problems, render hotspots, caching, bundle
optimization **Codebase:** `/client/apps/game/src`
...
`COSMETICS_SQL_ENDPOINT` computed at module load. If Torii base changes in-session (world switch without reload), cache
keys don't reflect the actual endpoint hit.
...
All major chunks (blockchain, ui-libs, external, three, main) load on initial page load. Landing page pays full game
download/parse cost before `/play`.

**Root Cause:** `manualChunks` in `vite.config.ts:97` defines chunk grouping but doesn't enable route-based code
splitting. Heavy imports are statically imported from entry graph.
...
_Report generated from combined analysis. All line numbers reference current codebase state and may shift with future
changes._
