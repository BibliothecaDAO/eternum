...
export const envSchema = z.object({
  ...
  VITE_PUBLIC_TORII_SUBSCRIPTION_SETUP_TIMEOUT_MS: z
    ...
    .default("8000")
    .transform((v) => Number(v))
    .transform((value) => (Number.isFinite(value) && value >= 0 ? value : 8000))
...
export function parseGameEnv(
  ...
  let env = envSchema.parse({
    ...importEnv,
    VITE_PUBLIC_AMM_ROUTER_ADDRESS: resolveLegacyAmmRouterAddress(rawEnv),
  });

  if (options?.selectedChain) {
    env = { ...env, VITE_PUBLIC_CHAIN: options.selectedChain };
  }
