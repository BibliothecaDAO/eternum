...
import { directMessageCreateSchema, directMessageHistoryQuerySchema, directMessageReadReceiptSchema, directMessageThreadQuerySchema, directMessageTypingSchema, } from "@bibliothecadao/types";
import { db } from "../../db/client";
import { directMessageReadReceipts, directMessages, directMessageThreads, directMessageTypingStates, } from "../../db/schema/direct-messages";
import { requirePlayerSession } from "../middleware/auth";
import { formatZodError } from "../utils/zod";
export const directMessageRoutes = new Hono();
...
directMessageRoutes.get("/threads", async (c) => {
    ...
    const participantFilter = identityFilters.length > 0
        ? or(...identityFilters)
    ...
    const threads = await query
        ...
        .limit(limit);
    const nextCursor = threads.length === limit
...
directMessageRoutes.post("/threads/:threadId/typing", async (c) => {
    ...
    if (!payload.isTyping) {
        await db
            ...
            .where(and(eq(directMessageTypingStates.threadId, payload.threadId), eq(directMessageTypingStates.playerId, payload.playerId)));
        return c.body(null, 204);
    }
...
export function buildThreadId(playerA, playerB) {
    const [first, second] = sortParticipants(playerA, playerB);
    return `${first}|${second}`;
}
export function sortParticipants(playerA, playerB) {
    return [playerA, playerB].sort((a, b) => (a < b ? -1 : a > b ? 1 : 0));
}
