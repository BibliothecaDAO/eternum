...
import type {
  AgentEvent,
  AgentRuntimeConfig,
  AgentWorldChatMetadata,
  AgentWorldSubscription,
  SteeringJobConfig,
...
export const agents = pgTable(
  "agents",
  {
    ...
    runtimeConfigJson: jsonb("runtime_config_json").$type<AgentRuntimeConfig>().notNull().default({}),
    subscriptionJson: jsonb("subscription_json").$type<AgentWorldSubscription[]>().notNull().default([]),
    ...
    steeringJobConfigJson: jsonb("steering_job_config_json").$type<SteeringJobConfig>().notNull().default({}),
...
export const agentEvents = pgTable(
  "agent_events",
  {
    ...
    payloadJson: jsonb("payload_json").$type<AgentEvent["payload"]>().notNull().default({}),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => ({
...
export const agentWorldChatRateLimits = pgTable("agent_world_chat_rate_limits", {
  ...
  metadataJson: jsonb("metadata_json").$type<AgentWorldChatMetadata | null>(),
});
