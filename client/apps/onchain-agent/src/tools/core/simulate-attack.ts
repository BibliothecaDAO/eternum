...
export async function simulateAttack(
  ...
  if (!tile || tile.occupierType === 0) {
    return { success: false, message: `Nothing to attack at (${input.targetX},${input.targetY}).`, combat: null, defenderLabel: null };
  }
  ...
  if (isExplorer(tile.occupierType)) {
    ...
    if (structure) {
      ...
      if (guard) {
        ...
        return { success: true, message: `${structure.category} at (${input.targetX},${input.targetY}) is unguarded — free capture.`, combat: null, defenderLabel: null };
      }
    }
  }

  if (!defenderInput || defenderInput.troopCount <= 0) {
    return { success: false, message: `No troops to fight at (${input.targetX},${input.targetY}).`, combat: null, defenderLabel: null };
  }
