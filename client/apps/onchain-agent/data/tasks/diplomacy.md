...
- Use `create_guild` if you are not in a guild and have at least 3 ticks of stable economy. Set `isPublic: false` to
  control membership.
...
- Use `update_whitelist` to grant trusted players access to your guild. Only whitelist players you have verified as
  allies through consistent non-aggression.
...
- Allies share a key benefit: mutual non-aggression frees troops from border defense, allowing more explorers and
  economic focus.
- Track ally behavior. If an ally moves armies toward your structures or raids your neighbors without coordination,
  reconsider the alliance.

## Hyperstructure Contributions

- Use `contribute_hyperstructure` to send surplus resources to allied hyperstructures. Contributions earn leaderboard
  points.
...
- Check hyperstructure access before contributing. Use `observe_game` to verify you have access or that the
  hyperstructure is public.

## Coordination Signals

- When sending resources to an ally via `send_resources`, note it in your reflection as a diplomatic action.
- When an ally is under attack (enemy armies near their structures), consider diverting an explorer to assist or raid
  the attacker's exposed structures.
- Avoid attacking structures belonging to whitelisted players or guild members unless explicitly re-evaluating the
  alliance.
...
- Hyperstructure contribution becomes high priority when you are falling behind on leaderboard points and have economic
  surplus.
