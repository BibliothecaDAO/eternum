...
description: authoritative guide on Three.js performance and best practices (2026). Use when writing, refactoring, or optimizing Three.js or React Three Fiber code. Covers WebGPU, memory management, and rendering optimization.
---

# Three.js Best Practices (2026)

## Instructions
When generating or reviewing Three.js code, strictly adhere to these guidelines to ensure performance and modern standards. Prioritize WebGPU-ready code (r171+) while maintaining WebGL 2 fallbacks.

## 1. WebGPU & Renderer
- **Init:** Always use `await renderer.init()` with `WebGPURenderer`. It enables zero-config WebGPU with auto WebGL 2 fallback.
- **TSL (Three Shader Language):** Write shaders in TSL (`three/tsl`) instead of GLSL/WGSL. It compiles to both backends.
...
## 4. Memory Management
- **Disposal:** Manually call `.dispose()` on **geometries, materials, textures, and render targets** when objects are removed.
- **Pooling:** Use object pooling for frequently spawned/despawned entities (e.g., bullets).
- **React Three Fiber:** Use `useLoader.preload` and `useGLTF.preload`. Never create objects ( `new THREE.Vector3`) inside `useFrame`.
