...
function sozo(args: string[], opts?: { label?: string; outDir?: string }) {
  ...
  // Log the exact command for CI diagnostics (safe in slot/test envs)
...
export async function maintainOrchestrator(p: Params) {
  ...
  // Trace credentials used (slot/test env OK)
  ...
  for (const entry of state) {
    ...
    // Log a short preview of the payload for debugging
    ...
    const cfgHash = sozo([
      ...
      "--private-key", pk,
      ...payload,
    ], { outDir: path.join(repoRoot, `contracts/game/factory/${chain}/calldata/${name}`), label: `configure:${name}` });
