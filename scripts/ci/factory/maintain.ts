...
export async function maintainOrchestrator(p: Params) {
  ...
  for (const entry of state) {
    ...
    log(
      `[CONFIG PAYLOAD:${name}] head="${payload.slice(0, 10).join(" ")}${payload.length > 10 ? " ..." : ""}" tokens=${payload.length}`,
    );
    const cfgHash = sozo(
      ["--profile", chain, "--rpc-url", rpcUrl, "--account-address", acct, "--private-key", pk, ...payload],
  ...
  for (const entry of state) {
    ...
    let world = "";
    try {
      world = await Bun.file(path.join(base, `world_address.txt`)).text();
    } catch {}
    if (world) {
      ...
      const curl = spawnSync(
        ...
        [
          ...
          "-X",
          "POST",
          `${tc}?env=${chain}&rpc_url=${rpcUrl}&torii_namespaces=${ns}&torii_prefix=${name}&torii_world_address=${world.trim()}`,
        ],
        { encoding: "utf-8" },
      );
