...
class ImageAnalyzer {
  ...
  generateReport(images) {
    ...
    this.stats.largeFiles
      ...
      .slice(0, 15)
      .forEach((file, index) => {
        console.log(`  ${index + 1}. ${file.relativePath} (${this.formatBytes(file.size)}) - ${file.type.toUpperCase()}`);
      });
    ...
    if (gifs.length > 0) {
      ...
      console.log(`  • ${gifs.length} GIF files (${this.formatBytes(gifsSize)}) - Consider converting to WebP or optimizing`);
    }
    ...
    const compressibleFiles = images.filter(img => 
      ['png', 'jpg', 'jpeg', 'gif'].includes(img.type) && img.size > 1024
    );
    ...
    console.log(`  • Potential bundle size reduction: ${((estimatedSavings / this.stats.totalSize) * 100).toFixed(1)}%`);
