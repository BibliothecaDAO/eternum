...
function getContributorStats(since) {
  const gitLog = executeGitCommand(
    `git log --since="${since}" --pretty=format:"%ae|%an|%s|%ad" --date=short`
  );
...
function getPackageChanges(since) {
  const packages = ['client', 'packages/core', 'packages/dojo', 'packages/provider', 
                  'packages/react', 'packages/torii', 'packages/types', 'contracts'];
  
  const packageChanges = {};
  
  packages.forEach(pkg => {
    const commits = executeGitCommand(
      `git log --since="${since}" --oneline -- ${pkg}/ | wc -l`
    );
