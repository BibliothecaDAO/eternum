...
mod BuildRoute {
    ...
    fn execute(realm_id: felt252, start_x: u32, start_y: u32, end_x: u32, end_y: u32) {
        ...
        assert(owner.address == player_id, 'Realm is not yours');

        let route = Segment { start_x: start_x, start_y: start_y, end_x: end_x, end_y: end_y,  };
        ...
        let construction_time: u128 = convert_u64_to_u128(starknet::get_block_timestamp());
        // create the route
        ...
        // TODO: better way to convert from u256 to felt252
...
mod Travel {
    ...
    fn execute(entity_id: felt252, routes: Array<Segment>) {
        // verify owner of entity_id
        let player_id: ContractAddress = starknet::get_caller_address();
        // entity id needs travel, position and owner component to travel
        let (travel, position, owner) = commands::<Travel,
        Position,
        Owner>::entity(entity_id.into());

        // assert owner
        assert(owner.address == player_id, 'Entity is not yours');
