...
const session = (address?: string | null) =>
  vi.fn(async () => Response.json({ session: { id: "session-1" }, user: { id: "u1", realmsId: "0x7", address } }));

describe("verified launcher identity", () => {
  test("forwards the session cookie and trusts only the linked wallet the identity Worker returns", async () => {
  ...
  test("gives an account without a linked wallet no launch identity", async () => {
    for (const address of [undefined, null]) {
      const resolver = createIdentityResolver("https://play.realms.party", session(address));
      expect(await Effect.runPromise(resolver.resolve("better-auth.session_token=signed"))).toBeNull();
    }
  });
});
