  ...
  Schema.decodeUnknownEffect(EnvironmentSchema)(environment).pipe(
    Effect.flatMap((raw) =>
      Effect.try(() => ({
        ...
        launcherAllowlist: new Set(
          raw.LAUNCHER_ALLOWLIST.split(",")
            .map((value) => value.trim())
            .filter(Boolean)
        ...
        rotationConfigs: (raw.LAUNCH_ROTATION_CONFIGS ?? "config/deployer/clean/launch-configs/madara-blitz-daily.yaml")
          .split(",")
