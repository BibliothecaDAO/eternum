...
The sample points browser traffic at the TLS endpoints served by `deploy/madara-lab/Caddyfile`. The lab chain is
fee-free, so the sample carries no master credentials.
