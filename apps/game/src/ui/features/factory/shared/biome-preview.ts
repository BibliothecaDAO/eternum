...
interface FactoryBiomeClimateStringValues {
...
interface BiomePreviewClimateInput {
  ...
  overrides: FactoryBiomeClimateStringValues;
}
...
const BIOME_PREVIEW_SIZE = 21;
const BIOME_PREVIEW_CENTER = 0;
...
export const buildFactoryBiomePreviewModel = (input: {
  baseClimate?: FactoryBiomeClimateValues;
  overrides: FactoryBiomeClimateStringValues;
}) => {
  const climate = buildBiomePreviewClimate(input);

  return buildBiomePreviewModel({
    climate,
