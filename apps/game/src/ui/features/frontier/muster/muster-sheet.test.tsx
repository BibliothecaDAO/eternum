...
// The home ring as the map knows it: explored and empty, or not yet synced.
const ring = { explored: true };
vi.mock("@/hooks/use-world-spatial-tiles", () => ({
  useWorldSpatialTiles: (hexes: { col: number; row: number }[]) =>
    ring.explored ? hexes.map((hexCoords) => ({ hexCoords, occupierId: 0 })) : [],
}));
...
describe("the muster sheet", () => {
  it("starts at the most the realm can field, musters the dragged count and closes", async () => {
    ...
    expect(host.querySelector('[aria-label^="Strength"]')).not.toBeNull();
