 ...
 * per-game model leads with `game_id` as key[0]. Set once during bootstrap,
 * before the Herald session starts or any system call is built.
 */
...
/** Fully-qualified model name for the active arm, e.g. `s2-TileOpt`. */
export const gameModel = (name: string): string => `${activeNamespace}-${name}`;

/**
 * Encode a model key as unpadded hex for Starknet and Dojo APIs.
 */
const hexKey = (value: number | bigint): string => `0x${BigInt(value).toString(16)}`;

/** The active game id as an unpadded model key. */
