...
describe("snapshotChunkDiagnostics", () => {
  it("returns a clone disconnected from future mutations", () => {
    ...
    diagnostics.terrainSourceReadyDurationMsSamples.push(9);
    diagnostics.terrainCommitDurationMsSamples.push(2);
    diagnostics.terrainFirstRenderedFrameDurationMsSamples.push(11);
    ...
    diagnostics.terrainSourceReadyDurationMsSamples.push(19);
    diagnostics.terrainCommitDurationMsSamples.push(7);
    diagnostics.terrainFirstRenderedFrameDurationMsSamples.push(26);
    ...
    expect(snapshot.terrainSourceReadyDurationMsSamples).toEqual([9]);
    expect(snapshot.terrainCommitDurationMsSamples).toEqual([2]);
    expect(snapshot.terrainFirstRenderedFrameDurationMsSamples).toEqual([11]);
...
describe("cloneChunkDiagnosticsBaselines", () => {
  it("clones entry objects and diagnostics payloads", () => {
    ...
    diagnostics.terrainFirstRenderedFrameDurationMsSamples.push(7);
    ...
    expect(cloned[0].diagnostics.terrainFirstRenderedFrameDurationMsSamples).not.toBe(
      source[0].diagnostics.terrainFirstRenderedFrameDurationMsSamples,
    );
  });
});
