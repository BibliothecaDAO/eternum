...
describe("worldmap cache budget policy", () => {
  it("drives worldmap prepared terrain cache capacity from policy recommended minimum", () => {
    const worldmapSource = readWorldmapSource();
    expect(worldmapSource).toMatch(
      /maxPreparedTerrainCacheSize\s*=\s*WORLDMAP_CHUNK_POLICY\.cache\.recommendedMinSize/,
    );
  });
