...
describe("handleWorldmapRefreshCommitRuntime", () => {
  ...
  it("drops stale prepared terrain", async () => {
  ...
  it("drops prepared terrain when the refresh is not committed but terrain was prepared", async () => {
  ...
  it("commits prepared terrain on a successful refresh", async () => {
    const result = await handleWorldmapRefreshCommitRuntime({
      chunkKey: "48,48",
      commitPreparedTerrain: vi.fn(),
      diagnostics: { id: "diagnostics" } as never,
