...
import {
  decodeRealmMetadata,
  readRealmMetadata,
} from "../../../apps/account-portal/src/lib/realms/metadata";

describe("decodeRealmMetadata", () => {
  it("decodes an on-chain base64 JSON data URI", () => {
    const encoded = Buffer.from(
      JSON.stringify({ name: "Skassluk", attributes: [] }),
    ).toString("base64");

    expect(
      JSON.parse(
        decodeRealmMetadata(`data:application/json;base64,${encoded}`),
      ),
    ).toEqual({ name: "Skassluk", attributes: [] });
  });

  it("normalizes an already-decoded JSON value", () => {
    expect(decodeRealmMetadata('{ "name": "Realm #17" }')).toBe(
      '{"name":"Realm #17"}',
    );
  });

  it("rejects non-JSON metadata", () => {
    expect(() => decodeRealmMetadata("ipfs://not-json")).toThrow(
      "Unsupported Realm metadata format",
    );
  });
});
...
describe("readRealmMetadata", () => {
  it("calls the on-chain metadata entrypoint and decodes its result", async () => {
    const encoded = Buffer.from(JSON.stringify({ name: "Realm #17" })).toString(
      "base64",
    );
    ...
    expect(calls).toEqual([
      { entrypoint: "get_decoded_metadata", calldata: ["17"] },
    ]);
