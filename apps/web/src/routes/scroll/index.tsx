...
import { formatScrollDate, getPublishedScrollPosts } from "@/site/lib/scroll";
...
function ScrollIndexPage() {
  ...
  const updateCount = useMemo(() => posts.filter((post) => post.type === "update").length, [posts]);
  const thoughtCount = useMemo(() => posts.filter((post) => post.type === "thought-piece").length, [posts]);
  const totalReadingMinutes = useMemo(() => posts.reduce((total, post) => total + post.readingTimeMinutes, 0), [posts]);
      ...
      >
        <header className="realm-panel realm-grid-scan scroll-codex-hero rounded-lg p-5 sm:p-7">
          <div className="flex flex-wrap items-end justify-between gap-5">
            <div className="max-w-3xl">
              <h1 className="realm-title text-3xl sm:text-4xl lg:text-5xl leading-tight">Scroll Archive</h1>
              <p className="mt-3 text-base sm:text-lg text-foreground/80">
                The written layer for Realms. Read ecosystem updates, design notes, and strategic thought pieces
                published as an open chronicle.
              </p>
            </div>
          ...
          <div className="scroll-codex-metrics mt-5">
            <article className="scroll-codex-metric">
              ...
              <p className="mt-2 text-2xl font-semibold text-primary tabular-nums">{updateCount}</p>
            </article>
            <article className="scroll-codex-metric">
              <p className="realm-sigil">Thought Pieces</p>
              <p className="mt-2 text-2xl font-semibold text-primary tabular-nums">{thoughtCount}</p>
            </article>
            <article className="scroll-codex-metric">
              <p className="realm-sigil">Reading Minutes</p>
              <p className="mt-2 text-2xl font-semibold text-primary tabular-nums">{totalReadingMinutes}</p>
            </article>
          </div>
        </header>
            ...
            {filterOptions.map((option) => (
              <button
                ...
                className={`scroll-codex-filter ${
                  filter === option.id ? "scroll-codex-filter-active" : "scroll-codex-filter-idle"
                }`}
              >
                {option.label}
              </button>
        ...
        {featuredPost ? (
          ...
          >
            ...
            <h2 className="realm-title mt-3 text-2xl sm:text-3xl lg:text-4xl leading-tight">{featuredPost.title}</h2>

            <p className="mt-3 text-foreground/80 text-base sm:text-lg max-w-3xl">{featuredPost.excerpt}</p>
        ...
        {feedPosts.length > 0 ? (
          <div className="scroll-codex-feed grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5">
            {feedPosts.map((post) => (
              <article key={post.slug} className="scroll-codex-card realm-panel rounded-lg p-5 md:p-6">
                ...
                <h3 className="text-xl md:text-2xl font-semibold mt-2 mb-2">
                  <Link to="/scroll/$slug" params={{ slug: post.slug }} className="scroll-codex-link transition-colors">
                    {post.title}
                  </Link>
                </h3>
