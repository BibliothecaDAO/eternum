...
export const Route = createFileRoute("/scroll/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      ...
      links: post.canonicalUrl ? [{ rel: "canonical", href: post.canonicalUrl }] : undefined,
    };
  },
});
      ...
      >
        <div className="mx-auto max-w-5xl space-y-6 sm:space-y-8">
          <header className="realm-panel realm-grid-scan scroll-detail-hero rounded-lg p-5 sm:p-7">
            ...
            <h1 className="realm-title mt-3 text-3xl md:text-5xl leading-tight">{post.title}</h1>
          ...
          <section className="realm-panel scroll-detail-body rounded-lg p-5 sm:p-8">
            <div className="scroll-content" dangerouslySetInnerHTML={{ __html: post.html }} />
          </section>
          ...
          <footer className="scroll-detail-neighbors grid grid-cols-1 md:grid-cols-2 gap-4">
            {newer ? (
              ...
              >
                <p className="text-xs uppercase tracking-[0.13em] text-foreground/55">Newer Entry</p>
            ...
            {older ? (
              <Link
                ...
                className="realm-panel scroll-detail-neighbor-card block rounded-lg p-4"
              >
                <p className="text-xs uppercase tracking-[0.13em] text-foreground/55">Older Entry</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {similarPosts.map((candidate) => (
                  ...
                  >
                    ...
                    <p className="text-lg font-semibold mt-1 line-clamp-2">{candidate.title}</p>
                    <p className="text-sm text-muted-foreground mt-2 line-clamp-3">{candidate.excerpt}</p>
                    <p className="text-xs text-muted-foreground mt-3">{formatScrollDate(candidate.date)}</p>
                  </Link>
                ))}
              </div>
            </section>
