...
describe("Realm bridge selection", () => {
  it("keeps the same selected token when a refreshed inventory changes order", () => {
    ...
    const before = createBridgeTable(
      [{ token_id: 1801 }, { token_id: 3324 }],
      selection,
    );
    const after = createBridgeTable(
      [{ token_id: 1101 }, { token_id: 3324 }, { token_id: 1801 }],
      selection,
    );

    expect(
      before.getFilteredSelectedRowModel().rows.map((row) => row.original),
    ).toEqual([{ token_id: 3324 }]);
    expect(
      after.getFilteredSelectedRowModel().rows.map((row) => row.original),
    ).toEqual([{ token_id: 3324 }]);
  });

  it("removes a selected token that disappears during refresh", () => {
    const refreshedRealms = [{ token_id: 1101 }];
    const selection = reconcileRealmBridgeSelection(
      { "3324": true },
      refreshedRealms,
    );
