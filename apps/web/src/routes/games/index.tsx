...
import type { Game } from "@/site/data/games";

export const Route = createFileRoute("/games/")({
  loader: async () => {
    const { games } = await import("@/site/data/games");
    return { games };
  },
  component: GamesPage,
  head: () => ({
...
const gamesDirectoryOrder = ["blitz", "realms-eternum", "loot-survivor", "zkube", "pistols-at-dawn"];

function orderGames(games: Game[]) {
  const gamesBySlug = new Map(games.map((game) => [game.slug, game]));
  return gamesDirectoryOrder.map((slug) => gamesBySlug.get(slug)).filter((game): game is Game => Boolean(game));
}
        ...
        >
          <header className="realm-panel realm-grid-scan realm-games-hero rounded-lg p-5 sm:p-7 lg:p-8">
            <div className="flex flex-wrap items-end justify-between gap-5">
              <div className="max-w-3xl">
                <h1 className="realm-title text-3xl leading-tight sm:text-4xl lg:text-5xl">Games Directory</h1>
                <p className="mt-3 text-base text-foreground/80 sm:text-lg">
                  Browse live and in-development games shipping across the Realms.World ecosystem. Access more
                  information, socials and gameplay links on individual game pages.
                </p>
              </div>
            ...
            <div className="realm-games-metrics mt-5 sm:!grid-cols-2">
              <article className="realm-games-metric">
                ...
                <p className="mt-2 text-2xl font-semibold text-primary tabular-nums">{liveGamesCount}</p>
              </article>
              <article className="realm-games-metric">
                <p className="realm-sigil">Aligned Studios</p>
                <p className="mt-2 text-2xl font-semibold text-primary tabular-nums">{studiosCount}</p>
              </article>
            </div>
          </header>
          ...
          {featuredGame ? (
            <motion.button
              ...
              onClick={() => navigate({ to: "/games/$slug", params: { slug: featuredGame.slug } })}
            ...
            >
              <div className="grid gap-5 lg:grid-cols-[1.15fr_1fr]">
                ...
                <div className="p-5 sm:p-6 lg:py-7">
                  ...
                  <p className="mt-5 text-sm font-bold leading-relaxed text-foreground/90 sm:text-base">{blitzGrab}</p>
                  <p className="mt-4 text-sm leading-relaxed text-foreground/78">{blitzFeaturedDetail}</p>
              ...
              {secondaryGames.map((game, index) => (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-4 sm:p-5">
                    ...
                    <p className="mt-1 text-xs uppercase tracking-[0.14em] text-foreground/55">{game.studio}</p>
                    ...
                    <div className="mt-4 flex flex-nowrap items-start justify-between gap-2">
                      ...
                      <span className="realm-sigil realm-studio-tag ml-auto shrink-0 text-[11px]">{game.studio}</span>
                    </div>
                  </div>
                </motion.button>
              ))}
