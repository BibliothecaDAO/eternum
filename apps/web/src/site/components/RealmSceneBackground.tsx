...
export function RealmSceneBackground() {
  ...
  useEffect(() => {
    ...
    const motionScale = window.matchMedia("(prefers-reduced-motion: reduce)").matches ? 0.15 : 1;
    ...
    const asciiQuad = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), asciiMaterial);
    ...
    for (let i = 0; i < ringCount; i++) {
      ...
      const color = new THREE.Color().lerpColors(new THREE.Color("#c08030"), new THREE.Color("#30a0b0"), t);
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const color =
        rnd < 0.5 ? colGreen.clone().lerp(colCyan, Math.random()) : rnd < 0.85 ? colCyan.clone() : colAmber.clone();
      pCol[i * 3] = color.r;
      pCol[i * 3 + 1] = color.g;
      pCol[i * 3 + 2] = color.b;
    ...
    for (let i = 0; i < pulseCount; i++) {
      const geo = new THREE.TorusGeometry(tunnelRadius * 0.7, 0.04, 4, isSmall ? 32 : 64);
      const mat = new THREE.MeshBasicMaterial({
        color: "#00ffaa",
        transparent: true,
        opacity: 0,
    ...
    const animate = () => {
      ...
      for (const pulse of pulses) {
        ...
        const fade = pulse.progress < 0.1 ? pulse.progress / 0.1 : Math.max(0, 1 - (pulse.progress - 0.1) / 0.9);
        pulse.mat.opacity = fade * 0.35;

        // Color shift: cyan → green as it approaches
        pulse.mat.color.setHSL(0.42 + pulse.progress * 0.08, 0.85, 0.5);
