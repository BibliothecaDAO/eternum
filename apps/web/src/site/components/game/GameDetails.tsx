...
import { Game } from "@/site/data/games";
...
export function GameDetails({ game }: { game: Game }) {
  ...
  const handleNextScreenshot = () => {
    if (selectedScreenshot !== null && game.backgroundImages) {
      setSelectedScreenshot((prev) => (prev !== null ? (prev + 1) % game.backgroundImages!.length : 0));
    }
  };
  ...
  const handlePreviousScreenshot = () => {
    if (selectedScreenshot !== null && game.backgroundImages) {
      setSelectedScreenshot((prev) =>
        prev !== null ? (prev === 0 ? game.backgroundImages!.length - 1 : prev - 1) : 0,
      );
    }
  };
  ...
  return (
    <>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 max-w-7xl rounded-lg realm-games-detail-shell">
        ...
        >
          <Button variant="rune" size="sm" onClick={() => navigate({ to: "/games" })} className="mb-5 sm:mb-6">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-2">
                    ...
                    <Badge
                      variant="outline"
                      className={`uppercase tracking-[0.12em] ${statusChipClasses(game.status)}`}
                    >
                      {game.status}
                    </Badge>
                  </div>
                ...
                <p className="text-base sm:text-lg leading-relaxed text-foreground/82">{game.description}</p>
          ...
          >
            ...
            {game.tvl ? (
              <article className="realm-panel realm-games-detail-stat rounded-lg p-4">
                ...
                <p className="mt-3 text-lg font-semibold text-foreground tabular-nums">${game.tvl.toLocaleString()}</p>
              </article>
            ) : null}
              ...
              {game.dashboards?.length ? (
                <div className={game.backgroundImages?.length ? "mb-6 sm:mb-7" : ""}>
                  <header className="mb-4 flex items-end justify-between gap-4">
                    ...
                    <span className="realm-sigil">{game.dashboards.length} dashboard</span>
                  </header>
                  ...
                  <header className="mb-4 flex items-end justify-between gap-4">
                    <div>
                      ...
                      <h2
                        className={
                          game.dashboards?.length
                            ? "realm-title text-2xl sm:text-3xl"
                            : "realm-title mt-3 text-2xl sm:text-3xl"
                        }
                      >
                        Screenshots
                      </h2>
                    </div>
                    <span className="realm-sigil">{game.backgroundImages.length} captures</span>
                  </header>
