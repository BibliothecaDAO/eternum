...
import { lordsInfoQueryOptions, treasuryBalanceQueryOptions } from "@/site/lib/query-options";
import { useVelords } from "@/site/hooks/use-velords";
import { LORDS_TOTAL_SUPPLY } from "@/site/lib/constants";
import { Button } from "@/site/components/ui/button";
import { ExternalLink, Coins, Landmark, TrendingUp } from "lucide-react";
import { LordsFlywheel } from "@/site/components/ui/LordsFlywheel";

export function EconomicsSection() {
  return <EconomicsSectionContent />;
}
...
function EconomicsSectionContent() {
  ...
  const lordsLockedUsd =
    Number.isFinite(parsedLordsPrice) && typeof lordsLocked === "number" ? lordsLocked * parsedLordsPrice : null;

  const heroMetrics = [
    {
      label: "LORDS Price",
      value: Number.isFinite(parsedLordsPrice) ? `$${parsedLordsPrice.toFixed(4)}` : "—",
      highlight: false,
    },
    {
      label: "Staking APY",
    ...
    <section className="realm-section container mx-auto px-4 py-16 md:py-24">
      ...
      >
        ...
        >
          ...
          <h2 className="realm-title text-2xl sm:text-3xl md:text-4xl">$LORDS TOKEN & TREASURY</h2>
          <p className="realm-subtitle text-base sm:text-lg">
            An ecosystem token designed for real-stakes gameplay economies and long-term studio alignment.
          </p>
        </motion.div>
        ...
        >
          {heroMetrics.map((metric, index) => (
            <div
              key={metric.label}
              className={`p-4 text-center sm:p-6 md:p-8 ${index > 0 ? "border-l border-primary/10" : ""}`}
            >
        ...
        <p className="mx-auto max-w-3xl text-center text-xs leading-relaxed text-foreground/55">
          * APY and fee figures use rolling 4-week averages from completed weekly epochs.
        </p>
        ...
        <LordsFlywheel
          metrics={{
            ...
            lordsLocked: lordsLocked
              ...
              : undefined,
            currentAPY: typeof currentAPY === "number" ? `${currentAPY.toFixed(1)}%*` : undefined,
          }}
        />
        ...
        >
          <div className="mx-auto space-y-4">
            <h3 className="realm-banner mx-auto flex w-fit text-center">TOKEN SUPPLY</h3>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {supplyStats.map((item) => (
                ...
                >
                  ...
                  <p className="text-lg font-bold tabular-nums tracking-tight text-foreground/95">{item.value}</p>
                  <p className="mt-1 text-[10px] text-foreground/60">{item.sub}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        >
          <div>
            ...
            <p className="mt-1 text-sm text-foreground/60">Lock LORDS as veLORDS and earn weekly staking rewards.</p>
          </div>
