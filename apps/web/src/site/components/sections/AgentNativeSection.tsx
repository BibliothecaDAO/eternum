...
import { Bot, Eye, Brain, Zap, ArrowRight, MessageSquare, RefreshCw, Swords, Globe } from "lucide-react";
...
const agentLoop = [
  ...
  },
  {
    icon: MessageSquare,
    title: "Report",
    description: "Send information, warnings, and strategic advice to a human player in real time.",
  },
  {
    icon: Brain,
    title: "Decide",
    description: "Score candidate moves and take action within strategic bounds.",
  },
  {
    icon: Zap,
    title: "Act",
    description: "Submit moves onchain. Every action is deterministic and auditable.",
  },
  {
    icon: RefreshCw,
    title: "Review",
    description: "Consider outcomes and learn from every victory and defeat.",
  },
];
          ...
          NODE_ORBIT_DEGS.forEach((nodeOrbit, nodeIndex) => {
            ...
            if (dist < PULSE_THRESHOLD && !cooldowns.has(nodeIndex)) {
              ...
              if (card) {
                gsap.to(card, {
                  scale: 1.06,
                  borderColor: "color-mix(in oklab, var(--primary) 65%, transparent)",
                  duration: 0.3,
                  ease: "power2.out",
                });
                gsap.to(card, {
              ...
              gsap.delayedCall(ORBIT_SECS / 5 - 0.2, () => cooldowns.delete(nodeIndex));
            }
          });
        });
      }
  ...
  return (
    <div ref={containerRef}>
      <div className="hidden md:block">
        <div className="relative mx-auto w-full max-w-[700px]" style={{ aspectRatio: "1 / 1" }}>
          <svg ref={svgRef} viewBox="0 0 400 400" className="absolute inset-0 h-full w-full" fill="none">
            <defs>
              <filter id="agent-loop-particle-glow" x="-100%" y="-100%" width="300%" height="300%">
            ...
            return (
              ...
              >
                <div className="al-node-card flex h-[140px] w-[168px] flex-col items-center justify-center rounded-lg border border-primary/20 bg-black/70 px-3 py-3 backdrop-blur-md">
                  ...
                  <p className="text-sm font-bold leading-tight tracking-wide">{step.title}</p>
                  <p className="mt-1 text-[11px] leading-snug text-foreground/55">{step.description}</p>
                </div>
              </div>
            );
          })}
      ...
      <div className="md:hidden">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2.5 pl-[3.25rem] text-sm text-primary/55">
            ...
            <span className="uppercase tracking-[0.12em]">Cycle repeats every match</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="realm-subtitle text-base sm:text-lg">
            Fully onchain games with the agentic era at the center of the design thesis.
          </p>
        </motion.div>
        ...
        >
          <p className="realm-banner mx-auto mb-8 flex w-fit text-center">THE AGENT LOOP</p>
          <AgentLoopFlywheel />
        </motion.div>
        ...
        >
          <p className="realm-banner mx-auto mb-8 flex w-fit text-center">FLAGSHIP AGENT-NATIVE GAMES</p>
              ...
              >
                ...
                <ul className="mb-5 flex-1 space-y-2.5">
                  {game.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2.5 text-sm leading-relaxed text-foreground/68">
