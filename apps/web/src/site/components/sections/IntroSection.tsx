...
import { Castle, ChevronDown, Coins, Gamepad2, TrendingUp } from "lucide-react";
...
const HeroApyValue = lazy(() =>
  import("@/site/components/sections/HeroApyValue").then((module) => ({
    default: module.HeroApyValue,
  })),
);

const RealmSceneBackground = lazy(() =>
  import("@/site/components/RealmSceneBackground").then((module) => ({
...
function IntroSectionContent() {
  ...
  const lordsPrice = Number.isFinite(parsedLordsPrice) ? `$${parsedLordsPrice.toFixed(4)}` : null;
    ...
    <section className="realm-section relative min-h-[92vh] overflow-hidden flex flex-col items-center justify-start">
      ...
      <div className="container mx-auto px-4 pb-16 pt-24 sm:pb-20 sm:pt-24 md:pb-24 md:pt-28">
        <div className="flex flex-col items-center text-center max-w-5xl mx-auto">
          ...
          >
            The AI era is here and fundamentally changing gaming as we know it. The Realms Ecosystem is building for the
            new age, one where AI and humans can build, strategize, collaborate, and compete. Together. Onchain.
            Forever.
          </motion.p>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {tickerStats.map((stat, index) => (
                ...
                >
                  ...
                  <p className="text-[10px] uppercase tracking-[0.14em] text-foreground/70">{stat.label}</p>
                  <p className="text-sm font-semibold text-foreground leading-tight">
                    {stat.label === "Staking APY" ? (
                      <Suspense fallback={<span className="text-foreground/40">&mdash;</span>}>
                        <HeroApyValue />
                      </Suspense>
                    ) : (
                      (stat.value ?? <span className="text-foreground/40">&mdash;</span>)
                    )}
                  </p>
                </motion.div>
              ))}
