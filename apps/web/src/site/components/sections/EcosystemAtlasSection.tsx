...
import { games } from "@/site/data/games";
    ...
    <section className="realm-section relative">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <h2 className="realm-title mb-4 text-2xl sm:text-3xl md:text-4xl">REALMS ECOSYSTEM GAMES</h2>
          <p className="realm-subtitle text-base sm:text-lg">
            Onchain games built for players, builders, and AI agents. Compete through verifiable rules and transparent
            economies.
          </p>
          <Link
            to="/games"
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-8 p-6 sm:p-8 lg:p-10 min-h-[340px] sm:min-h-[400px]">
            ...
            <div className="flex flex-col justify-end">
              ...
              >
                ...
                <h3 className="realm-title text-3xl sm:text-4xl md:text-5xl leading-tight mb-3">Realms: Blitz</h3>
                <p className="text-base sm:text-lg text-foreground/85 max-w-xl mb-6 leading-relaxed">
                  A fast-paced onchain RTS where humans and AI agents execute tactics in real time. One-hour matches are
                  fully verified on Starknet, with top-ranking players winning a share of the $LORDS prize pool.
                </p>
                ...
                <div className="flex flex-wrap gap-3 mb-6">
                  ...
                  <Button size="lg" variant="oath" asChild>
                    <Link to="/games/$slug" params={{ slug: "blitz" }}>
                      Learn More
                    </Link>
                  </Button>
                </div>
              ...
              {blitzFeatures.map((feature, index) => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-foreground/80 leading-tight">{feature.detail}</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div className="relative z-10 lg:hidden grid grid-cols-2 gap-2 px-6 pb-6 sm:px-8 sm:pb-8">
            {blitzFeatures.map((feature) => (
              ...
              >
                ...
                <p className="text-[10px] font-semibold leading-tight">{feature.label}</p>
              </div>
            ))}
          </div>
        </motion.div>
            ...
            >
              ...
              <div className="relative flex h-full flex-col justify-end p-4 sm:p-5">
                ...
                <div className="absolute top-3 right-3">
                  {game.isLive ? (
                    <span className="realm-sigil text-green-400 border-green-500/30">Live</span>
                  ) : (
                    <span className="realm-sigil text-foreground/65">Coming Soon</span>
                  )}
                </div>

                <h3 className="text-xl font-semibold mb-1.5">{game.title}</h3>
                <p className="text-xs text-foreground/80 mb-2.5 line-clamp-2">{game.description}</p>
                ...
                <div className="mb-3 flex flex-nowrap items-start justify-between gap-2">
                  <div className="flex min-w-0 flex-wrap gap-1.5">
                    {game.genre
                      ...
                      .slice(0, 3)
                      .map((genre) => (
                        <span key={`${game.slug}-${genre}`} className="realm-sigil text-[9px]">
                          {genre}
                        </span>
                      ))}
                  </div>
                  <span className="realm-sigil realm-studio-tag ml-auto shrink-0 text-[9px]">{game.studio}</span>
                </div>
