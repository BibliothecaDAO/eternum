...
export function drawScene(ctx: CanvasRenderingContext2D, state: GameState, w: number, h: number, now: number): void {
  const halfW = w / 2;
  const halfH = h / 2;
...
function drawHexGrid(
  ...
  for (const hex of allHexes) {
    ...
    const fogAlpha = dist > state.gridRadius - 5 ? Math.max(0, 1 - (dist - (state.gridRadius - 5)) / 5) : 1;
...
function drawTrail(ctx: CanvasRenderingContext2D, state: GameState, offsetX: number, offsetY: number): void {
  ctx.globalAlpha = 1;
...
function drawDeathFlash(
  ...
  const flashAlpha = elapsed < 200 ? Math.min(elapsed / 100, 0.6) : Math.max(0, 0.6 - (elapsed - 200) / 1000);
...
function drawVignette(ctx: CanvasRenderingContext2D, w: number, h: number, halfW: number, halfH: number): void {
  ctx.save();
