...
export function HexExplorerUI({ activeFeature, activeAgent, agentScreenPos, showHint, isMobile }: HexExplorerUIProps) {
  return (
        ...
        {showHint && (
          ...
          >
            <div className="realm-panel px-5 py-3 text-center pointer-events-auto">
              <p className="text-sm tracking-[0.1em] uppercase" style={{ fontFamily: "var(--font-mono)" }}>
        ...
        {activeFeature && (
          ...
          >
            <div className="realm-panel p-5">
              ...
              <p className="text-sm text-foreground/70 leading-relaxed mb-3">{activeFeature.description}</p>
              {activeFeature.link && <FeatureLink link={activeFeature.link} type={activeFeature.type} />}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function FeatureLink({ link, type }: { link: string; type: FeatureHexData["type"] }) {
