    ...
    const ctx = gsap.context(() => {
        ...
        particles.forEach((p) => {
          ...
          NODE_ORBIT_DEGS.forEach((nod, ni) => {
            ...
            if (dist < PULSE_THRESHOLD && !cooldowns.has(ni)) {
              ...
              gsap.delayedCall(ORBIT_SECS / 4 - 0.2, () => cooldowns.delete(ni));
            }
          });
        });
      }
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          tl.to(arrows, { opacity: 1, duration: 0.3, stagger: 0.07 }, "-=0.25");
    ...
    >
      ...
      <div className="text-center mb-6 sm:mb-8">
        ...
        <p className="text-base sm:text-lg text-foreground/65 max-w-xl mx-auto leading-relaxed">
          A self-reinforcing loop — game activity drives staking rewards, staking rewards incentivize more play.
        </p>
      </div>

      {/* ── Desktop: animated circular flywheel ── */}
      <div className="hidden md:block">
        <div className="relative w-full max-w-[580px] mx-auto" style={{ aspectRatio: "1/1" }}>
          <svg ref={svgRef} viewBox="0 0 400 400" className="absolute inset-0 w-full h-full" fill="none">
            <defs>
              <filter id="fw-particle-glow" x="-100%" y="-100%" width="300%" height="300%">
          ...
          {STEPS.map((step, i) => {
            ...
            const metricValue = step.metricKey && metrics?.[step.metricKey];
            ...
            return (
              ...
              >
                <div className="fw-node-card flex h-[136px] w-[160px] flex-col items-center justify-center rounded-lg border border-primary/20 bg-black/70 px-3 py-3 backdrop-blur-md">
                  ...
                  <p className="text-sm font-bold leading-tight tracking-wide">{step.label}</p>
                  {metricValue && <p className="text-base font-bold text-primary tabular-nums mt-1">{metricValue}</p>}
                  <p className="text-[11px] text-foreground/50 leading-snug mt-1">{step.detail}</p>
                </div>
              </div>
            );
          })}
      ...
      <div className="md:hidden">
        <div className="flex flex-col gap-4">
            ...
            return (
              ...
              >
                ...
                <div className="pt-2">
                  ...
                  {metricValue && <p className="text-sm font-bold text-primary tabular-nums mt-0.5">{metricValue}</p>}
                  <p className="text-sm text-foreground/60 leading-snug mt-1">{step.detail}</p>
                </div>
              </div>
            );
          })}
          <div className="flex items-center gap-2.5 pl-[3.25rem] text-sm text-primary/55">
            ...
            <span className="uppercase tracking-[0.12em]">Cycle repeats weekly</span>
          </div>
        </div>
      </div>
    </div>
