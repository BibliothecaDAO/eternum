...
import { CONTRACTS, TIME_CONSTANTS, APY_CONSTANTS } from "@/site/lib/constants";

import { REWARD_POOL_ABI } from "@/site/lib/abis/rewardPool";
import { VELORDS_ABI } from "@/site/lib/abis/velords";
import { LORDS_ABI } from "@/site/lib/abis/lords";
import { lordsInfoQueryOptions } from "@/site/lib/query-options";
...
export const useVelords = () => {
  ...
  const [rollingWeekOne, rollingWeekTwo, rollingWeekThree, rollingWeekFour] = rollingWeekStarts;
  const rollingWeekOneSnapshot = useWeeklyRewardPoolSnapshot(rollingWeekOne, !!chain);
  const rollingWeekTwoSnapshot = useWeeklyRewardPoolSnapshot(rollingWeekTwo, !!chain);
  const rollingWeekThreeSnapshot = useWeeklyRewardPoolSnapshot(rollingWeekThree, !!chain);
  const rollingWeekFourSnapshot = useWeeklyRewardPoolSnapshot(rollingWeekFour, !!chain);
  ...
  console.log("[useVelords] Total supply:", {
    totalSupply,
    totalSupplyError,
    formatted: totalSupply !== undefined ? formatTokenAmount(toBigInt(totalSupply)) : "N/A",
  });
  ...
  console.log("[useVelords] LORDS in veLORDS:", {
    lordsInVelords,
    lordsInVelordsError,
    formatted: lordsInVelords !== undefined ? formatTokenAmount(toBigInt(lordsInVelords)) : "N/A",
  });
  ...
  // Calculate TVL
  const tvl = useQuery({
    queryKey: ["velords-tvl", lordsInVelords?.toString(), lordsPrice?.price?.rate],
    queryFn: () => {
      ...
      const price =
        typeof lordsPrice.price.rate === "string" ? parseFloat(lordsPrice.price.rate) : lordsPrice.price.rate;
  ...
  console.log("[useVelords] Tokens this week:", {
    tokensThisWeek,
    tokensThisWeekError,
    formatted: tokensThisWeek !== undefined ? formatTokenAmount(toBigInt(tokensThisWeek)) : "N/A",
  });
  ...
  console.log("[useVelords] Tokens last week:", {
    ...
    formatted: tokensLastWeek !== undefined ? formatTokenAmount(toBigInt(tokensLastWeek)) : "N/A",
  });

  // Get veLORDS supply for this week
  const { data: veSupplyThisWeek, error: veSupplyThisWeekError } = useReadContract({
    abi: REWARD_POOL_ABI,
    functionName: "get_ve_supply",
    address: CONTRACTS.STARKNET.REWARD_POOL,
    args: [currentWeek],
  ...
  console.log("[useVelords] VE supply this week:", {
    ...
    formatted: veSupplyThisWeek !== undefined ? formatTokenAmount(toBigInt(veSupplyThisWeek)) : "N/A",
  });

  // Get veLORDS supply for last week
  const { data: veSupplyLastWeek, error: veSupplyLastWeekError } = useReadContract({
    abi: REWARD_POOL_ABI,
    functionName: "get_ve_supply",
    address: CONTRACTS.STARKNET.REWARD_POOL,
    args: [safeLastWeek],
  ...
  console.log("[useVelords] VE supply last week:", {
    ...
    weekQueried: safeLastWeek,
    formatted: veSupplyLastWeek !== undefined ? formatTokenAmount(toBigInt(veSupplyLastWeek)) : "N/A",
  });
  ...
  console.log("[useVelords] User balance:", {
    userBalance,
    userBalanceError,
    formatted: userBalance !== undefined ? formatTokenAmount(toBigInt(userBalance)) : "N/A",
  });
  ...
  const rollingFourWeekAverages = useQuery({
    ...
    queryFn: () => {
      ...
      const totalTokens = snapshots.reduce((sum, snapshot) => sum + toBigInt(snapshot.tokens), 0n);
      const totalVeSupply = snapshots.reduce((sum, snapshot) => sum + toBigInt(snapshot.veSupply), 0n);
      ...
      const averageAPY =
        averageVeSupply > 0 ? (averageWeeklyRewards / averageVeSupply) * APY_CONSTANTS.BLOCKS_PER_YEAR * 100 : 0;
  ...
  const userWeeklyRewards = useQuery({
    ...
    queryFn: () => {
      ...
      if (userBalance === undefined || tokensThisWeek === undefined || veSupplyThisWeek === undefined) {
        console.log("[useVelords] User rewards calculation skipped - missing data");
        return 0;
      }

      const veSupplyBigInt = toBigInt(veSupplyThisWeek);
      if (veSupplyBigInt === 0n) {
        console.log("[useVelords] User rewards calculation failed - zero supply");
        return 0;
      }

      const userShare = (toBigInt(userBalance) * toBigInt(tokensThisWeek)) / veSupplyBigInt;
    ...
    enabled: userBalance !== undefined && tokensThisWeek !== undefined && veSupplyThisWeek !== undefined,
    staleTime: 60000, // Cache for 1 minute
  });
  ...
  console.log("[useVelords] Rolling 4w APY calculation result:", rollingFourWeekAverages.data);

  const result = {
    // Supply data
    totalSupply: totalSupply !== undefined ? formatTokenAmount(toBigInt(totalSupply)) : undefined,
    veSupplyThisWeek: veSupplyThisWeek !== undefined ? formatTokenAmount(toBigInt(veSupplyThisWeek)) : undefined,
    lordsLocked: lordsInVelords !== undefined ? formatTokenAmount(toBigInt(lordsInVelords)) : undefined,

    // Rewards data
    tokensThisWeek: tokensThisWeek !== undefined ? formatTokenAmount(toBigInt(tokensThisWeek)) : undefined,
    tokensLastWeek: tokensLastWeek !== undefined ? formatTokenAmount(toBigInt(tokensLastWeek)) : undefined,
    ...
    // User data
    userBalance: userBalance !== undefined ? formatTokenAmount(toBigInt(userBalance)) : undefined,
