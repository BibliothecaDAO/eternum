...
import { abbreviateNumber, formatNumber, SUPPORTED_L2_CHAIN_ID } from "@/utils/utils";
import { useAccount, useBalance, useContract, useReadContract, useSendTransaction } from "@starknet-start/react";
...
export const StakeLords = () => {
  ...
  const totalLords = useMemo(() => {
    return Number(formatEther(BigInt(ownerLordsLock?.amount ?? 0n))) + availableLords;
  }, [availableLords, ownerLordsLock?.amount]);
  ...
  return (
    <Card className="flex flex-col">
      <CardHeader className="pb-0">
        ...
        <CardDescription>Your balances of locked and unlocked $LORDS</CardDescription>
      </CardHeader>
      {address ? (
        <>
          <div className="flex flex-col items-center sm:flex-row">
            <Card className="mt-4 sm:ml-8 sm:mt-0">
              ...
              <CardFooter className="text-muted-foreground px-4 pb-3 text-sm">Lords Available</CardFooter>
            </Card>
            <ChartContainer config={chartConfig} className="relative h-[180px] w-full">
              <RadialBarChart
                accessibilityLayer
                data={chartData}
                endAngle={180}
              ...
              >
                <ChartTooltip
                  cursor={false}
                  content={<ChartTooltipContent hideLabel className="ml-2 min-w-[180px]" />}
                />
                      ...
                      if (viewBox && "cx" in viewBox && "cy" in viewBox) {
                        return (
                          <text x={viewBox.cx} y={viewBox.cy} textAnchor="middle">
                            <tspan x={viewBox.cx} y={viewBox.cy + 15} className="fill-foreground text-lg font-bold">
                              {ownerLordsLock?.amount
                                ? abbreviateNumber(formatEther(BigInt(ownerLordsLock.amount)))
                                : "0"}{" "}
                              /{abbreviateNumber(totalLords)}
                            </tspan>
                            <tspan x={viewBox.cx} y={viewBox.cy + 32} className="fill-muted-foreground">
                              Staked / Total $LORDS
                            </tspan>
                          </text>
                        );
                ...
                <RadialBar dataKey="unlocked" stackId="a" fill="var(--color-chart-5)" cornerRadius={5} />
                <RadialBar order={0} dataKey="locked" stackId="a" fill="var(--color-chart-1)" cornerRadius={5} />
              </RadialBarChart>
            </ChartContainer>
          </div>
          <CardFooter className="flex-col gap-2 text-sm">
            <div className="mb-4 flex gap-2">
              ...
              />
              <UnlockDialog ownerLordsLock={ownerLordsLock} lordsBalance={availableLords} withdraw={withdraw} />
            </div>
            <div className="flex items-center gap-2 font-medium leading-none">
              veLords are entitled to a share of Lords fees from ecosystem games and marketplaces. One lock per wallet
              address.
            </div>
            <div className="text-muted-foreground leading-none">
              Note: max 75% penalty for early withdrawal (if withdrawn immediately after locking for 4 years)
            </div>
          </CardFooter>
        </>
      ) : (
