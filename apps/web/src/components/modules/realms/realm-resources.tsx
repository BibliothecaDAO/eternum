...
export default function RealmResources({ traits }: { traits: TokenMetadataAttribute[] }) {
  ...
  return (
    <div className="flex items-center gap-2">
      {resources.slice(0, visibleCount).map((resource, index) => (
        <div key={index} className="bg-secondary flex items-center gap-2 rounded-lg p-2">
          {resource.value && <ResourceIcon size="md" resource={resource.value.toString()} />}
        </div>
      ))}
        ...
        <>
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="h-16 max-w-48 p-2" id="resource-tooltip" side="top">
                <div className="flex items-center gap-2">
                  {resources.slice(visibleCount).map((resource, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <div>
                        ...
                        {resource.value && (
                          <ResourceIcon size="md" withTooltip={false} resource={resource.value.toString()} />
                        )}
                      </div>
                    </div>
                  ))}
