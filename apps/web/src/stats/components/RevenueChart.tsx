...
const RevenueChart: React.FC<RevenueChartProps> = ({ revenueData, lordsPrice, totalLords }) => {
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: function (context: TooltipItem<"doughnut">) {
            ...
            const lordsFormatted =
              isMobile && value >= 1000 ? `${(value / 1000).toFixed(0)}K LORDS` : `${value.toLocaleString()} LORDS`;

            const usdFormatted =
              isMobile && usdValue >= 1000000
                ? `≈ $${(usdValue / 1000000).toFixed(1)}M USD`
                : `≈ $${usdValue.toLocaleString()} USD`;

            return [lordsFormatted, `${percentage}% of total revenue`, usdFormatted];
          },
        },
      },
    },
  ...
  const formatUSD = (lordsAmount: number): string => {
    ...
    return isMobile && usdValue >= 1000000 ? `${(usdValue / 1000000).toFixed(1)}M` : usdValue.toLocaleString();
  };

  const formatLords = (amount: number): string => {
    return isSmallMobile && amount >= 1000 ? `${(amount / 1000).toFixed(0)}K` : amount.toLocaleString();
  };

  const getTotalDisplayValue = (): string => {
    return isSmallMobile && totalLords >= 1000 ? `${(totalLords / 1000).toFixed(0)}K` : totalLords.toLocaleString();
  };
              ...
              <div className="legend-item-left">
                ...
                <div className="legend-info">
                  ...
                  <div className="legend-subtitle">
                    {isMobile ? `${item.percentage}%` : `${item.percentage}% of total revenue`}
                  </div>
                </div>
              </div>
