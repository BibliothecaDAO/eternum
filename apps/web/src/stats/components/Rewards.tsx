...
const Rewards: React.FC<RewardsProps> = ({ lordsPrice, strkPrice, initialTab }) => {
  ...
  const [selectedRewardType, setSelectedRewardType] = useState<"victory" | "cartridge" | "daydreams" | "chests">(
    initialTab || "victory",
  );
  ...
  const handleTabChange = (tab: "victory" | "cartridge" | "daydreams" | "chests") => {
    ...
    void navigate({ to: "/stats/rewards/$tab", params: { tab } });
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [eternumResponse, cartridgeResponse, knownAddressesResponse, nexus6Response, chestResponse] =
          await Promise.all([
            fetch("/stats/data/eternum-social-export.json"),
            fetch("/stats/data/cartridge-points.json"),
            fetch("/stats/data/known-addresses.json"),
            fetch("/stats/data/nexus-6-players.json"),
            fetch("/stats/data/chest-rewards.json").catch(() => ({ ok: false, json: () => [] })),
          ]);
  ...
  const getAllPlayers = (): (Player & {
    tribeName: string;
    tribeRank: number;
    tribePrize: { lords: number; strk: number };
  })[] => {
  ...
  const renderCartridgePrizes = () => {
    ...
    const filteredRewards = cartridgeRewards.filter((reward) => {
      ...
      return reward.address.toLowerCase().includes(search) || (playerName && playerName.toLowerCase().includes(search));
    });
        ...
        <div className="daydreams-header">
          <div className="daydreams-title">
            ...
            <p className="daydreams-subtitle">
              25,000 STRK equally distributed among {daydreamsRewards.length} players who completed the NEXUS-6
              achievement (killed 10 AI agents)
            </p>
          </div>
          ...
          <div className="daydreams-summary">
            ...
            <div className="summary-card">
              ...
              <div className="summary-value">
                {daydreamsRewards.length > 0 ? formatStark(daydreamsRewards[0].strkReward) : "0"} STRK
              </div>
              <div className="summary-usd">
                {daydreamsRewards.length > 0 ? formatStrkUSD(daydreamsRewards[0].strkReward) : "$0"}
              </div>
            </div>
          </div>
        </div>
  ...
  const renderChestRewards = () => {
    ...
    const filteredRewards = chestRewards.filter((reward) => {
      ...
      return (
        reward.toAddress.toLowerCase().includes(search) || (playerName && playerName.toLowerCase().includes(search))
      );
    });
                ...
                <div key={bracket.chests} className="distribution-item">
                  ...
                  <div className="distribution-details">
                    <div className="distribution-range">
                      {bracket.min.toLocaleString()}-{bracket.max.toLocaleString()} pts
                    </div>
                    <div className="distribution-players">
                      {playerCount} players ({percentage.toFixed(0)}%)
                    </div>
                  </div>
                </div>
              );
                ...
                <div key={`${reward.toAddress}-${index}`} className="player-row">
                  ...
                  <div className="reward-info">
                    <div className="reward-amount">
                      {reward.count} {reward.count === 1 ? "Chest" : "Chests"}
                    </div>
            ...
            {players.map((player, index) => (
              <div key={`${player.address}-${index}`} className="player-row">
                ...
                <div className="tribe-info">
                  <div className="tribe-name">{player.tribeName}</div>
                  <div className="tribe-rank">Rank #{player.tribeRank}</div>
                  <div className="tribe-prize">
                    Tribe Pool: {formatLords(player.tribePrize.lords)} LORDS ({formatLordsUSD(player.tribePrize.lords)})
                    / {formatStark(player.tribePrize.strk)} STRK ({formatStrkUSD(player.tribePrize.strk)})
                  </div>
                </div>
                ...
                <div className="reward-breakdown">
                  <div className="breakdown-item">
                    ...
                    <span className="breakdown-value">
                      {formatLords(player.rewards.memberShare)} LORDS ({formatLordsUSD(player.rewards.memberShare)}) /{" "}
                      {formatStark(player.rewards.memberShareStrk)} STRK (
                      {formatStrkUSD(player.rewards.memberShareStrk)})
                    </span>
                  </div>
                  {player.isOwner && (
                    <div className="breakdown-item owner-bonus">
                      ...
                      <span className="breakdown-value">
                        {formatLords(player.rewards.ownerBonus)} LORDS ({formatLordsUSD(player.rewards.ownerBonus)}) /{" "}
                        {formatStark(player.rewards.ownerBonusStrk)} STRK (
                        {formatStrkUSD(player.rewards.ownerBonusStrk)})
                      </span>
                    </div>
                  )}
                  <div className="breakdown-item">
  ...
  return (
    <div className="rewards-container">
          ...
          <div className="season-totals-grid">
            <div className="total-card lords-card">
              <div className="total-icon">
                <img src="/stats/Lords.svg" alt="LORDS Token" className="lords-icon" />
              </div>
            ...
            <div className="total-card strk-card">
              <div className="total-icon">
                <img src="/stats/strk.png" alt="STRK Token" className="strk-icon" />
              </div>
              <div className="total-info">
          ...
          <div className="season-breakdown">
            <div className="breakdown-item">
              ...
              <span className="breakdown-description">
                Distributed based on tribe rankings and player contributions
              </span>
            </div>
            <div className="breakdown-item">
              <span className="breakdown-category">🎮 Achievement Rewards</span>
              <span className="breakdown-description">
                300K LORDS for cartridge achievements, 25K STRK for Nexus-6 achievement
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="rewards-content">{renderRewardTypeContent()}</div>
    </div>
  );
};
