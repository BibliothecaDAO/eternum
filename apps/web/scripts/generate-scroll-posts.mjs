...
`;
  ...
  process.stdout.write(`Generated ${path.relative(rootDir, outputFile)} with ${posts.length} posts\n`);
}

generate().catch((error) => {
  process.stderr.write(`Failed to generate scroll posts: ${error instanceof Error ? error.message : String(error)}\n`);
  process.exit(1);
});
