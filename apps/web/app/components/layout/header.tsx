...
export function Header() {
  ...
  return (
    <header className={cn("flex sticky  z-50 top-0 shrink-0 items-center transition-[width,height] ease-linear  border-b bg-sidebar", )}>
      <div className="flex w-full gap-2 h-[--header-height]">
        <div className={`${open ? "w-[--sidebar-width]" : "w-[--sidebar-width-icon]"}` + " border-r flex items-center justify-center"}>
          <Link to="/">
            <RWLogo className={`${open ? "w-20" : "w-10"}` +" h-auto transition-[width,height] duration-300"} />
          </Link>
        </div>
