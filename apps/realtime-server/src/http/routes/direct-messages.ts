...
export function createDirectMessageRoutes(
  notifications: DirectMessageNotificationPublisher = DISABLED_DIRECT_MESSAGE_NOTIFICATIONS,
) {
  ...
  directMessageRoutes.post("/messages", async (c) => {
    ...
    publishDirectMessageNotification(notifications, player, result.success.message);
