...
export const useEntryTokenBalance = ({
  ...
  const registerComponent = setup?.components?.BlitzEntryTokenRegister;

  const getEntryTokenIdByIndex: EntryTokenBalanceResult["getEntryTokenIdByIndex"] = useCallback(
    async (ownerAddress = account?.address, options, indexOverride) => {
    ...
    [account?.address, entryTokenAddress, registerComponent, starknetProvider],
  );
