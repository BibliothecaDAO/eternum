...
/*
 * Manual fixups applied on top of codegen output — preserve on regeneration:
 *   • u256 fields must be declared as a nested `{ low, high }` schema instead of
 *     `RecsType.BigInt`. Torii encodes u256 as a Cairo struct; `@dojoengine/state`'s
 *     `handleBigInt` cannot coerce a struct into a BigInt and logs
 *     "Failed to convert [object Object] to BigInt". Declaring the nested schema
 *     lets `convertValues` recurse via `handleDefault` into the u128 halves.
 *   Affected fields (keep in sync with any new u256 fields added to Cairo models):
 *     - SeasonPrize.total_lords_pool
 *     - WorldConfig.blitz_registration_config.fee_amount
 */
