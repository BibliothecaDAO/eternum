...
const HASH_DOMAIN_SELECTORS = new Map(schemaRegistry.hashDomains.map((name) => [name, hash.getSelectorFromName(name)]));
...
export function getHashDomainSelector(name: string): string {
  const selector = HASH_DOMAIN_SELECTORS.get(name);
  if (!selector) throw new Error(`unregistered hash domain: ${name}`);
  return selector;
}
