...
export const getClosestBank = (
  entityId: ID,
  components: ClientComponents,
): ClosestBank | undefined => {
  ...
  const closestBank = banksArray.reduce((closest: { bankId: ID, distance: number, travelTime?: number } | null, entity: Entity) => {
    ...
    return distance < closest.distance 
      ? { bankId: bankStructure.entity_id, distance, travelTime } 
      : closest;
  }, null);
