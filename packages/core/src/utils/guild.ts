...
export const formatGuilds = (
	...
	const guildMember = getComponentValue(
...
export const getGuild = (
	...
	return formatGuilds(
		[getEntityIdFromKeys([BigInt(guildEntityId)])],
		playerAddress,
		components,
	)[0];
};
...
export const formatGuildMembers = (
	...
	return guildMembers
		...
		.filter(
			(guildMember): guildMember is NonNullable<typeof guildMember> =>
...
export const getGuildMembersFromPlayerAddress = (
	...
	return formatGuildMembers(
		[getEntityIdFromKeys([BigInt(guildMember.guild_id)])],
		playerAddress,
		components,
	);
};
