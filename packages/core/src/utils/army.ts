...
export const formatArmies = (
	...
	return armies
		.map((armyEntity) => {
			...
			const totalCapacityKg = getArmyTotalCapacityInKg(
				explorerTroops.explorer_id,
				components,
			);

			const resource = getComponentValue(components.Resource, armyEntity);
			const weightKg = resource
				? gramToKg(divideByPrecision(Number(resource.weight.weight)))
				: 0;
			...
			const structure = getComponentValue(
				...
				getEntityIdFromKeys([BigInt(explorerTroops.owner)]),
			);
			...
			return {
				...
				ownerName:
					getAddressNameFromEntity(explorerTroops.owner, components) || "",
				structure,
				isMine,
				isMercenary,
				isHome,
...
export const armyHasTraveled = (
	entityArmies: ArmyInfo[],
	realmPosition: { x: number; y: number },
) => {
	return entityArmies.some(
		(army) =>
			army &&
			realmPosition &&
			(army.position.x !== realmPosition.x ||
				army.position.y !== realmPosition.y),
	);
};

export const getTroopName = (
	troopType: TroopType,
	troopTier: TroopTier,
): string => {
	switch (troopTier) {
...
export const getTroopResourceId = (
	troopType: TroopType,
	troopTier: TroopTier,
): ResourcesIds => {
	switch (troopType) {
...
export const getGuardsByStructure = (
	structure: ComponentValue<ClientComponents["Structure"]["schema"]>,
) => {
	if (!structure?.troop_guards) return [];

	const guardResurrectionDelay =
		configManager.getTroopConfig().troop_limit_config.guard_resurrection_delay;
	...
	const guards = [
		{
			...
			// timestamp
			cooldownEnd:
				structure.troop_guards.delta_destroyed_tick * armiesTickInSeconds +
				guardResurrectionDelay,
		},
		{
			slot: 1,
			...
			cooldownEnd:
				structure.troop_guards.charlie_destroyed_tick * armiesTickInSeconds +
				guardResurrectionDelay,
		},
		{
			slot: 2,
			...
			cooldownEnd:
				structure.troop_guards.bravo_destroyed_tick * armiesTickInSeconds +
				guardResurrectionDelay,
		},
		{
			slot: 3,
			...
			cooldownEnd:
				structure.troop_guards.alpha_destroyed_tick * armiesTickInSeconds +
				guardResurrectionDelay,
		},
	];
...
export const isArmyAdjacentToStructure = (
	...
	return adjacentHexes.some(
		(hex) => hex.col === armyPosition.x && hex.row === armyPosition.y,
	);
};

export const getFreeDirectionsAroundStructure = (
	...
	const adjacentHexes = getNeighborHexes(
		structure.base.coord_x,
		structure.base.coord_y,
	);
	...
	adjacentHexes.forEach((hex) => {
		const tile = getComponentValue(
			components.Tile,
			getEntityIdFromKeys([BigInt(hex.col), BigInt(hex.row)]),
		);
...
export const getRemainingCapacityInKg = (
	...
	const weight = getComponentValue(
		components.Resource,
...
export const getArmyTotalCapacityInKg = (
	...
	const totalCapacity = getComponentValue(
		components.Resource,
		getEntityIdFromKeys([BigInt(entityId)]),
	)?.weight.capacity;
