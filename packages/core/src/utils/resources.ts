...
export const getInventoryResources = (
	entityId: ID,
...
export const getBalance = (
	...
	const resourceManager = new ResourceManager(components, entityId);
	return {
		balance: resourceManager.balanceWithProduction(
			currentDefaultTick,
			resourceId,
		),
		resourceId,
...
export const getResourceProductionInfo = (
	entityId: ID,
	resourceId: ResourcesIds,
	components: ClientComponents,
) => {
...
export const getQuestResources = (
	...
	const resourcesProduced = structure
		? unpackValue(structure.resources_packed)
		: [];
...
export const uniqueResourceInputs = (
	...
	for (const resourceProduced of resourcesProduced) {
		for (const resourceInput of resourceProductionInputResources[
			resourceProduced
		]) {
...
export const applyInputProductionFactor = (
	...
	for (const resourceInput of uniqueResourceInputs(
		resourcesOnRealm,
		resourceProductionInputResources,
	).filter((id) => id != ResourcesIds.Wheat && id != ResourcesIds.Fish)) {
		for (const questType in questResources) {
			questResources[questType] = questResources[questType].map(
				(questResource) => {
...
export const getStartingResources = (
	...
	return applyInputProductionFactor(
...
export const canTransferMilitaryResources = (
	fromEntityId: ID,
	toEntityId: ID,
	components: ClientComponents,
) => {
	const fromStructure = getComponentValue(
		components.Structure,
		getEntityIdFromKeys([BigInt(fromEntityId)]),
	);

	const toStructure = getComponentValue(
		components.Structure,
		getEntityIdFromKeys([BigInt(toEntityId)]),
	);
