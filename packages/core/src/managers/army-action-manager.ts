...
export class ArmyActionManager {
	...
	constructor(
		...
		const entityOwnerId = getComponentValue(
	...
	private _getTroopType(): TroopType {
		const entityArmy = getComponentValue(
			this.components.ExplorerTroops,
			this.entity,
		);
	...
	private readonly _calculateMaxTravelPossible = (
		currentDefaultTick: number,
		currentArmiesTick: number,
	) => {
		const stamina = this.staminaManager.getStamina(currentArmiesTick);
		...
		const maxStaminaSteps = Math.floor(
			Number(stamina.amount) / minTravelStaminaCost,
		);
		...
		const { wheat, fish } = this.getFood(currentDefaultTick);
		const maxTravelWheatSteps = Math.floor(
			wheat / travelFoodCosts.wheatPayAmount,
		);
	...
	public getFood(currentDefaultTick: number) {
		const wheatBalance = this.resourceManager.balanceWithProduction(
			currentDefaultTick,
			ResourcesIds.Wheat,
		);
		const fishBalance = this.resourceManager.balanceWithProduction(
			currentDefaultTick,
			ResourcesIds.Fish,
		);
	...
	public findActionPaths(
		...
		const armyStamina =
			this.staminaManager.getStamina(currentArmiesTick).amount;
		...
		for (const { col, row } of neighbors) {
			const isExplored =
				exploredHexes.get(col - FELT_CENTER)?.has(row - FELT_CENTER) || false;
			const hasArmy =
				armyHexes.get(col - FELT_CENTER)?.has(row - FELT_CENTER) || false;
			const isArmyMine =
				armyHexes.get(col - FELT_CENTER)?.get(row - FELT_CENTER)?.owner ===
					playerAddress || false;
			const hasStructure =
				structureHexes.get(col - FELT_CENTER)?.has(row - FELT_CENTER) || false;
			const isStructureMine =
				structureHexes.get(col - FELT_CENTER)?.get(row - FELT_CENTER)?.owner ===
					playerAddress || false;
			const biome = exploredHexes
				.get(col - FELT_CENTER)
				?.get(row - FELT_CENTER);
		...
		while (priorityQueue.length > 0) {
			...
			if (
				!lowestStaminaUse.has(currentKey) ||
				staminaUsed < lowestStaminaUse.get(currentKey)!
			) {
				lowestStaminaUse.set(currentKey, staminaUsed);
				const isExplored =
					exploredHexes
						.get(current.col - FELT_CENTER)
						?.has(current.row - FELT_CENTER) || false;
				const hasArmy =
					armyHexes
						.get(current.col - FELT_CENTER)
						?.has(current.row - FELT_CENTER) || false;
				...
				for (const { col, row } of neighbors) {
					...
					const isExplored =
						exploredHexes.get(col - FELT_CENTER)?.has(row - FELT_CENTER) ||
						false;
					const hasArmy =
						armyHexes.get(col - FELT_CENTER)?.has(row - FELT_CENTER) || false;
					const hasStructure =
						structureHexes.get(col - FELT_CENTER)?.has(row - FELT_CENTER) ||
						false;
					const biome = exploredHexes
						.get(col - FELT_CENTER)
						?.get(row - FELT_CENTER);
					...
					if (
						!lowestStaminaUse.has(neighborKey) ||
						nextStaminaUsed < lowestStaminaUse.get(neighborKey)!
					) {
						priorityQueue.push({
							position: { col, row },
		...
		this.components.Resource.addOverride(overrideId, {
			...
			value: {
				...
				weight: {
					...weight,
					weight:
						BigInt((weight?.weight || 0).toString()) + BigInt(additionalWeight),
				},
			},
		});
	...
	private readonly _optimisticTileUpdate = (
		...
		const previousEntity = getEntityIdFromKeys([
			BigInt(previousPosition.col),
			BigInt(previousPosition.row),
		]);

		const previousTile = getComponentValue(
		...
		const newEntity = world.registerEntity({
			id: getEntityIdFromKeys([
				BigInt(newPosition.col),
				BigInt(newPosition.row),
			]),
		});
		// createEntity(world, [
		//   this.components.Tile,
	...
	private readonly _optimisticExplore = (
		...
		const removeTileOverride = this._optimisticTileUpdate(
			newPosition,
			previousPosition,
		);
		const removeCapacityOverride = this._optimisticCapacityUpdate(
			...
			configManager.getExploreReward() *
				configManager.getResourceWeightKg(ResourcesIds.Wood),
		);
		const removeFoodCostsOverride = this._optimisticFoodCosts(
			TravelTypes.Explore,
		);
	...
	private readonly _exploreHex = async (
		...
		const { removeOverrides } = this._optimisticExplore(
			path[1].hex.col,
			path[1].hex.row,
			currentArmiesTick,
		);
	...
	private readonly _optimisticTravelHex = (
		...
		for (const { biomeType } of path) {
			...
			staminaCost += configManager.getTravelStaminaCost(
				biomeType,
				this._getTroopType(),
			);
		}
	...
	private readonly _optimisticFoodCosts = (travelType: TravelTypes) => {
		...
		const removeWheatResourceOverride =
			this.resourceManager.optimisticResourceUpdate(
				ResourcesIds.Wheat,
				-costs.wheatPayAmount,
			);
		const removeFishResourceOverride =
			this.resourceManager.optimisticResourceUpdate(
				ResourcesIds.Fish,
				-costs.fishPayAmount,
			);
