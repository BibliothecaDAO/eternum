...
export interface CreateGameClientInput {
  ...
  /**
   * Makes the client's sync runtime. By default the client owns a private one, so several clients live in one
   * process; the web app passes installFreshGameSyncRuntime so its scenes find its one client's runtime as the
   * active one.
   */
