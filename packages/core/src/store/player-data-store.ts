...
export class PlayerDataStore {
  ...
  public static getInstance(refreshInterval: number, sqlApi: SqlApi): PlayerDataStore {
    if (!PlayerDataStore.instance) {
      PlayerDataStore.instance = new PlayerDataStore(refreshInterval, sqlApi);
    }
  ...
  private async fetchAndStoreData(): Promise<void> {
    const result = await this.sqlApi.fetchGlobalStructureExplorerAndGuildDetails();
    let realmsData = realms as Record<string, { name: string }>;
    await Promise.all(
      result.map((item: PlayersData) => {
        let transformedItem = {
        ...
        transformedItem.explorerIds.forEach((explorerId: string) => {
        ...
        transformedItem.structureIds.forEach((structureId: string) => {
