...
maintenance, training, stamina, travel, raid decay, Share-seconds or season finalisation. Quotes and execution must use
the same contract clock semantics.
...
Do not hand-copy the 1,991 parameters or 1,362 recipe components into scattered constants. Build one deterministic
...
4. **Logistics:** Markets, Donkeys, freight, arrivals, loot delivery and capacity-aware partial rules.
5. **Military:** training escrow, troop tiers, deployment limits, stamina, combat, raids and capture settlement.
...
- Protected floors survive raids and Worker-only T1 recovery remains possible.
...
- Capture settles accrued VP, production, work and raid state before ownership changes.
