...
**Migration contract (adjudicated Aug 17 — build these first; they are what the channels migrate onto):**

- **Baseline-delta evidence for resource intents.** Exact `matchPatch` matching cannot settle resource writes —
  production and concurrent spends shift balances between prediction and echo (`provisional-write-manager.ts:200`). Add
  a second evidence mode: capture the touched fields' baseline at intent creation; a post-hash authoritative write where
  a touched field differs from that baseline settles the write. Exact matching stays for coords and levels.
- **Observable outcomes.** The manager exposes a session-owned subscription for intent outcomes (`settled` / `failed` /
  `stalled`). FX, building buttons, upgrade UI, and ghosts consume it. No window events, no side stores. This replaces
  the deleted handle getters — it is the one status surface.
- **Selectable lock duration.** `lockUntil: "transaction-hash" | "settled"` per intent. Movement keeps hash-unlock for
  chaining; realm upgrades lock until the authoritative level echo — that is what makes deleting the upgrade store safe
  against duplicate submissions.
- **Create-army settles on projection evidence.** The manager matches known entity IDs only — no predicate matching gets
  added. The creation intent keys on the producing structure (lock + failure); the ghost's settle evidence stays
  renderer-side, reading authoritative projection occupancy at the target hex. Sanctioned explicitly — it reads
  authority; it is not a side store.
- **Expiry lives at the chokepoint.** At the stalled timeout the manager reports telemetry, then fails the intent and
  removes its overlays; the failed outcome reaches subscribers, so ghosts and FX clear with it. The FX-side 45/90s
  timers delete. One timer, one owner.
...
| Component-local optimistic scalars, incl. a double (scalar + override for the same balance) and a store mutation that hides a relic forever if its tx fails                                                                    | `craft-relic-popup.tsx:192`, `game-review-modal.tsx:94`, `use-realm-store.ts:251`                                          | MIGRATE — except `game-review-modal`, which runs outside any game-sync session and cannot use the manager: wait for claim confirmation, update its legitimate landing-page query cache, delete the persistent optimistic Set |
...
`resource-arrivals-manager.ts:18-93`); `TileOpt` + `Structure` + `ResourceArrival` overridable wrapping
(`create-client-components.ts:11,13,15` — TileOpt/Structure never had a producer, and deleting `optimisticOffload`
removes ResourceArrival's only one; this is the same wrapper class that caused the P6A regression); the orphaned
...
**Gate:** `addOverride` appears only in the sync adapter (grep); the optimistic-cleanup waiter
(`resolveTransactionWaiter`/`extractTransactionHash`) exists exactly once, in the manager — legitimate non-session
confirmation helpers are out of scope; the PR carries the census — every channel above named as migrated or deleted,
nothing "kept for now".
