...
- **Accounts.** Superseded by `gameplay-account-login-brief.md`: identity wallet on mainnet through starknet-react,
  gameplay through a browser-held OpenZeppelin account deployed fee-free on the game chain. No static account list, no
  predeployed connector, no Controller, no paymaster.
