...
7. **Parity controls time, identities and randomness.** Pin the old rules revision, preset, starting IDs and timestamps.
   ...
   metadata normalization in advance. Recorded sequencer entropy can feed those same raw roots without changing the
   derivation. Named purposes and explicit counters come after parity in a versioned random-rules change with test
   vectors.
...
Define the protected actor entrypoint with sequencing-submitter authorization, internal domain commands and generated
schemas/bindings for `@bibliothecadao/eternum/game-client`. Every persistent mutation emits RowSet, RowMemberSet or
RowDeleted. Components carry layout notes, and historical events retain the metadata needed by downstream consumers.
...
The randomness track uses secure entropy generated by the trusted sequencing service, with action/order/root bound in
one replicated acceptance record. It requires no dedicated VRF key or verifier. Bindings belong to the logical service
and survive replacement of the active machine through durable replication and fenced failover. Replay uses recorded
roots. Operator honesty remains trusted; no production value uses draws before protocol review and integration gates.
