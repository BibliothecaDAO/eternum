...
| Camera movement performs zero Torii calls in game-wide mode. | `legacy bounded sync adapter > performs zero Torii work for camera movement in game-wide mode`; this proof must move to the surviving camera path before the legacy adapter is deleted. |
