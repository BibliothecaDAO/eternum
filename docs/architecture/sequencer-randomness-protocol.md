...
`STALE_RELEASE`, it holds signatures until the pin changes, refreshes the manifest's release catalogue if needed, and
signs once more with that pair. The catalogue is checked against the compiled decoder without downloading schema bytes.
Transient manifest failures retry; an unchanged pin fails with `STALE_RELEASE_UNCHANGED`. A release without the client's
compiled decoder is refused as `UNKNOWN_RELEASE_SCHEMA`; a hotfix with the same schema remains usable.
...
TypeScript, Rust and Cairo share `tests/fixtures/v6.txt` for canonical encoding, identities, draw vectors and per-game
epoch roots, and `context-v2.txt` for time/order boundaries. The same protocol assertions run against the stub and
native Games contract. They cover account authentication before nonce consumption, malformed transport, delayed
execution, monotonic time, batches mixing two games, atomic rollback, per-game heads, restart rerolls and epoch reveals.
The compiled ABI check includes the sequencing account.
