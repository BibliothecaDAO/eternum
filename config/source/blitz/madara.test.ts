...
describe("generated Madara Blitz config", () => {
  test("keeps the phase-one game collectible-free", () => {
    ...
    expect(config.setup.addresses).toMatchObject({
      strk: "0x4718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d",
    });
  });

  test.each([undefined, null, ""])("rejects missing address %p instead of writing zero", (address) => {
    expect(() => resolveConfiguredAddress(address, "lords")).toThrow("lords address is not configured");
  });
});
