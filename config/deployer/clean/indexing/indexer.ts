...
function buildWorkflowDispatchBody(request: IndexerRequest, dispatchId: string, ref: string) {
  return JSON.stringify({
    ...
    inputs: {
      ...
      runtime_provider: request.runtimeProvider,
      torii_name: request.worldName,
      runtime_instance_id: request.runtimeInstanceId || request.runtimeOwner?.runtimeInstanceId,
      torii_tier: request.tier || "basic",
      image_digest: request.imageDigest,
      request_json: JSON.stringify({
        version: request.toriiVersion,
        rpcUrl: request.rpcUrl,
        worldAddress: request.worldAddress,
        worldBlock: request.worldBlock || "0",
