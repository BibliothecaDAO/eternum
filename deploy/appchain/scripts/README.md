...
```bash
cd contracts/game
sozo build && sozo migrate --profile appchain
...
`s2_blitz-registrar_systems` resource before it can initialize `ChainConfig`.
...
for example `contracts/game/manifest_spike.json` during local verification; the tracked appchain manifest remains the
default.

The old `factory-config.ts` and `factory-create-game.ts` scripts are retained only as A2 migration history. They must not
be used for `s2_blitz` launches.
