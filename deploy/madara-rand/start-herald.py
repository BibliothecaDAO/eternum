...
def main():
    ...
    for database in databases:
        run([*command, 'exec', '-T', primary, 'psql', '-U', 'postgres', '-v', 'ON_ERROR_STOP=1',
    ...
    environment.write_text('\n'.join([
        ...
        f'HERALD_DATABASE_URL=postgres://postgres:local-rehearsal@{primary}:5432/{databases[0]}',
        f'HERALD_REPLAY_DATABASE_URL=postgres://postgres:local-rehearsal@{primary}:5432/{databases[1]}', '',
    ]))
    ...
    print(json.dumps({'environment': str(environment), 'scope': 'native Herald over the gameplay fixture'}))
